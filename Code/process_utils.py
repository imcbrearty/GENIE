...
class SeismicEmbeddingEngine:
    def __init__(
        ...
        # ---------------------------------------------------------------------
        # 1. Setup Graph Edges & Travel Times
        # ---------------------------------------------------------------------
        self.x_grid = x_grid
        ...
        # ---------------------------------------------------------------------
        # 2. Update Picks Matrix
        # ---------------------------------------------------------------------
        self.update_picks(P)

        # ---------------------------------------------------------------------
        # 3. Precompute Global Envelopes
        # ---------------------------------------------------------------------
    ...
    def _build_global_embedding(self):
        """Precomputes station-level continuous embedding matrices."""
        abs_time_ref = self.T_start + np.arange(self.n_time_series) * self.dt
        ...
        def build_channel_embed(ifind):
            if len(ifind) == 0:
                return torch.zeros((self.n_stations, self.n_time_series), device=self.device)
            ...
            vals = (imask * np.exp(-0.5 * (time_vals**2) / (self.kernel_sig_t**2))).reshape(-1)

            sta_ids = self.P[ifind, 1].astype(int)
            write_indices = (indices_clamped + sta_ids.reshape(-1, 1) * self.n_time_series).reshape(-1)

            embed_1d = scatter(
                torch.as_tensor(vals, dtype=torch.float32, device=self.device),
                torch.as_tensor(write_indices, dtype=torch.long, device=self.device),
                dim=0,
                dim_size=total_size,
                reduce="max",
            )
            return embed_1d.view(self.n_stations, self.n_time_series)

        # Separate phase envelopes
        self.embed_p = build_channel_embed(ifind_p)
        self.embed_s = build_channel_embed(ifind_s)

        self.embed_p[:, 0], self.embed_p[:, -1] = 0.0, 0.0
        self.embed_s[:, 0], self.embed_s[:, -1] = 0.0, 0.0

        # Combined envelope (all phase types regardless of phase ID)
        self.embed_all = torch.maximum(self.embed_p, self.embed_s)
    ...
    def extract_inputs(self, t0, min_t=0.0, max_t=300.0, t_win=10.0):
        ...
        # 1. Expected travel time arrivals per edge
        ...
        # 2. Extract Channels:
        # Col 0: P moveout checked against ALL picks (Phase type ignored)
        col0 = self.embed_all[self.sta_ids_gpu, clamp_p] * valid_p
        # Col 1: S moveout checked against ALL picks (Phase type ignored)
        col1 = self.embed_all[self.sta_ids_gpu, clamp_s] * valid_s

        # Col 2: P moveout checked against P picks ONLY
        col2 = self.embed_p[self.sta_ids_gpu, clamp_p] * valid_p
        # Col 3: S moveout checked against S picks ONLY
        col3 = self.embed_s[self.sta_ids_gpu, clamp_s] * valid_s

        val_embed = torch.stack((col0, col1, col2, col3), dim=1)

        Inpts = [val_embed]
        Masks = [1.0 * (torch.abs(val_embed) > 0.01)]

        # 3. Pick Extraction Window
