...
def extract_input_from_data(trv_pairwise, P, t0, ind_use, locs, x_grid, A_src_in_sta, trv_times = None, max_t = 300.0, kernel_sig_t = 5.0, dt = 0.2, batch_grids = False, use_asserts = True, verbose = False, use_sign_input = False, return_embedding = False, device = 'cpu'): ## pred_params[1]
	...
	if batch_grids == True:
		...
		if use_sign_input == True: ## Note, for differences, should account for the different windows
			val_embed_p = val_embed_p*torch.sign(torch.diff(embed, append = embed[[-1]] + (embed[[-1]] - embed[[-2]]), dim = 0))[trv_read_ind_p] ## Pointwise multiply by slope of embedding
			val_embed_s = val_embed_s*torch.sign(torch.diff(embed, append = embed[[-1]] + (embed[[-1]] - embed[[-2]]), dim = 0))[trv_read_ind_s] ## Pointwise multiply by slope of embedding
			val_embed_p1 = val_embed_p1*torch.sign(torch.diff(embed_p, append = embed_p[[-1]] + (embed_p[[-1]] - embed_p[[-2]]), dim = 0))[trv_read_ind_p] ## Pointwise multiply by slope of embedding
			val_embed_s1 = val_embed_s1*torch.sign(torch.diff(embed_s, append = embed_s[[-1]] + (embed_s[[-1]] - embed_s[[-2]]), dim = 0))[trv_read_ind_s] ## Pointwise multiply by slope of embedding
