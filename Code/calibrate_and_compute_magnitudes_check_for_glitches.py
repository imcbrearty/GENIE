...
if check_for_spikes == True:
	z = np.load('/scratch/groups/beroza/CentralCalifornia/example_glitches_ver_1.npz')
	spike_times, spike_dates = z['srcs_t'], z['dates']
	z.close()
	# Matched_spikes = []
	Srcs_spikes = []
	Dates_spikes = []
	tree_spikes = cKDTree(spike_dates)
	cnt_glitches = 0
...
for i in range(len(days)):
	...
	if check_for_spikes == True:
		ip_spike = tree_spikes.query_ball_point(date.reshape(1,-1), r = 0)[0]
		if len(ip_spike) > 0:
			temporal_match_glitch = 10.0
			spatial_match_glitch = np.inf
			## Find matched events
			glitch_events = srcs.mean(0, keepdims = True).repeat(len(ip_spike), axis = 0)
			glitch_events[:,3] = spike_times[ip_spike,0]
			matches_glitch = maximize_bipartite_assignment(glitch_events, srcs, ftrns1, ftrns2, temporal_win = temporal_match_glitch, spatial_win = spatial_match_glitch, verbose = True)[0]
			if len(matches_glitch) > 0:
				cnt_glitches += len(matches_glitch)
				print('Matched glitch %d (total %d)'%(len(matches_glitch), cnt_glitches))
				Srcs_spikes.append(srcs[matches_glitch[:,1]])
				Dates_spikes.append(date.reshape(1,-1).repeat(len(matches_glitch), axis = 0))
...
## Merge spikes
if (check_for_spikes == True)*(len(Srcs_spikes) > 0):
	Srcs_spikes = np.vstack(Srcs_spikes)
	Dates_spikes = np.vstack(Dates_spikes)
	plt.pause(1)
	print('\nNumber of glitches: %d'%(len(Srcs_spikes)))
	Store_results['Number glitches'] = len(Srcs_spikes)
...
Store_results['Results1'] = 'Detected %d/%d (%0.2f) events'%(len(Matches), len(srcs_ref), len(Matches)/len(srcs_ref))
Store_results['Results2'] = 'Lat residual %0.3f (+/- %0.3f)'%(res[:,0].mean(), res[:,0].std())
Store_results['Results3'] = 'Lon residual %0.3f (+/- %0.3f)'%(res[:,1].mean(), res[:,1].std())
Store_results['Results4'] = 'Depth residual %0.3f (+/- %0.3f)'%(res[:,2].mean(), res[:,2].std())
Store_results['Results5'] = 'Origin Time residual %0.3f (+/- %0.3f) \n'%(res[:,3].mean(), res[:,3].std())
...
for mag in mag_levels:
	...
	Store_results['Results_M%0.2f'%mag] = 'Detected %d/%d (%0.2f) events'%(len(ip1), len(ip), len(ip1)/len(ip))


ratio_events = len(srcs)/len(srcs_ref)
print('Number of sources %d'%len(srcs))
print('Number p picks %d'%sum(cnt_p))
print('Number s picks %d'%sum(cnt_s))
print('Ratio of events %0.3f'%(ratio_events))
Store_results['Ratio events'] = ratio_events
...
## Compute GR-offset over large magnitude range, and linearity of the GR curve above a specified threshold

d_mag = np.minimum(0.25, np.diff(np.linspace(np.minimum(mag_pred.min(), mag_ref.min()), np.maximum(mag_pred.max(), mag_ref.max()), 10))[0])
mag_range = np.arange(np.minimum(mag_pred.min(), mag_ref.min()), np.maximum(mag_pred.max(), mag_ref.max()) + d_mag, d_mag)
mag_fit_range = [1.75, 3.5] ## Misfit of GR curves over this range 
mag_linearity_thresh = 1.0 ## Calculate linearity of GR curve above this range

N, N1 = [], []
for i in range(len(mag_range)):
	N.append(np.log10(len(np.where(mag_pred > mag_range[i])[0])))
	N1.append(np.log10(len(np.where(mag_ref > mag_range[i])[0])))

i1, i2 = np.argmin(np.abs(mag_range - mag_fit_range[0])), np.argmin(np.abs(mag_range - mag_fit_range[1]))
f1 = np.polyfit(mag_range[i1:i2], N[i1:i2], 1)
f2 = np.polyfit(mag_range[i1:i2], N1[i1:i2], 1)
mag_eval = np.linspace(mag_range[i1], mag_range[i2], 30)
val1, val2 = np.polyval(f1, mag_eval), np.polyval(f2, mag_eval)
offset_vert = np.median(val1 - val2)
offset_horiz = -1.0*offset_vert/np.mean([f1[0], f2[0]]) ## Convert vertical offset to mag offset using the average slope (account for negative slope)
print('\nMagnitude offset: %0.3f M (%0.3f log_10[N])'%(offset_horiz, offset_vert))


## Compute density factor of catalog
k_neighbor = 30
tree = cKDTree(ftrns1(srcs))
dist = tree.query(ftrns1(srcs), k = k_neighbor + 1)[1][:,1::]
mean_dist = (dist.mean(1)/1000.0)
density_factor = np.mean(1.0/mean_dist)
print('\nDensity factor of full catalog: %0.3f'%density_factor)
# dist = np.linalg.norm(np.expand_dims(ftrns1(srcs)

## Compute density factor near Parkfield
lat_specific = [35.35, 36.5]
lon_specific = [-121.0, -119.5]
ifind = np.where((srcs[:,0] < lat_specific[1])*(srcs[:,0] > lat_specific[0])*(srcs[:,1] > lon_specific[0])*(srcs[:,1] < lon_specific[1]))[0]
tree = cKDTree(ftrns1(srcs[ifind]))
dist = tree.query(ftrns1(srcs[ifind]), k = k_neighbor + 1)[1][:,1::]
mean_dist_specific = (dist.mean(1)/1000.0)
density_factor_specific = np.mean(1.0/mean_dist_specific)
print('\nDensity factor near Parkfield: %0.3f'%density_factor_specific)

## Compute location consistency between GNN and travel time locations
r2_match = 0.5*r2_score(srcs_r[:,0], srcs[:,0]) + 0.5*r2_score(srcs_r[:,1], srcs[:,1])
print('\nLocation consitency (R^2): %0.4f'%(r2_match))

## Compute ratio of events with offsets > min_thresh
max_offset = 30e3
ifind = np.where(np.linalg.norm(ftrns1(srcs) - ftrns1(srcs_r), axis = 1) > max_offset)[0]
print('\nRatio Location offset: %0.4f'%(len(ifind)/len(srcs)))
