...
def kmeans_partition(p_prob, n_clusters, n_batch = 3000, n_steps = 1000, n_sim = 1, n_dim = 1, lr = 0.01):
	...
	for n in range(n_sim):
		...
		for i in range(n_steps):
			if i == 0:
				v = np.random.choice(len(p_prob), p = p_prob, size = n_clusters).reshape(-1,1)

			tree = cKDTree(v)
			x = np.random.choice(len(p_prob), p = p_prob, size = n_batch).reshape(-1,1)
			q, ip = tree.query(x)
			...
			for j in range(len(ipu)):
				...
				rs.append(np.linalg.norm(update)/np.sqrt(n_dim))
...
def MCL_pruning(srcs, ftrns1, sig_t = 10, sig_x = 50e3, use_directed = True):

	srcs_tensor = torch.Tensor(srcs).to(device)
	tree_t = cKDTree(srcs[:,3].reshape(-1,1))
	tree_x = cKDTree(ftrns1(srcs[:,0:3]))
	lp_t = tree_t.query_ball_point(srcs[:,3].reshape(-1,1), r = sig_t)
	lp_x = tree_x.query_ball_point(ftrns1(srcs[:,0:3]), r = sig_x)
	cedges = [np.array(list(set(lp_t[i]).intersection(lp_x[i]))) for i in range(len(lp_t))]
	cedges1 = np.hstack([i*np.ones(len(cedges[i])) for i in range(len(cedges))])
	edges = torch.Tensor(np.concatenate((np.hstack(cedges).reshape(1,-1), cedges1.reshape(1,-1)), axis = 0)).long().to(device)
	...
	G = nx.Graph()
	G.add_edges_from(list(np.concatenate((edges[0][:,None].cpu().detach().numpy(), edges[1][:,None].cpu().detach().numpy()), axis = 1)))
	lp = list(nx.connected_components(G))

	xp = []

	print('MCL not finished')

	for i in range(len(lp)):

		vec = np.array(lp[i])
		xx = np.zeros((len(lp[i]), len(lp[i])))
		

		# xx[]
		# pwd_x = pd(xx[edges[0,np.array(list(lp[i]))], edges[0,np.array(list(lp[i]))]]

		if xx.shape[0] > n_limit:

			# BP = Backprojection(data, Trv, dt = 0.25, max_t = T.max()*1.15)

			data_chunk = BP.data.extract_slice(ind_arvs)
			bp = Backprojection(data_chunk, BP.Trv, dt = BP.dt, max_t = BP.max_t, device = device)
			K = bp(xx, sig = sig).cpu().detach().numpy()
			K1 = bp.joint_prob(xx, xx, sig = sig).cpu().detach().numpy()

			Trns = K.copy()[:,None]*K1.copy()

			for j in range(Trns.shape[0]):

				# median or mean here?
				# Should we "learn" how to do optimal reduction?
				Trns[j,j] = np.mean(Trns[:,j])

			Trns = Trns/np.sum(Trns, axis = 0)[None,:]
			...
			for j in range(max_iter):
				...
				m2 = np.matmul(m1,m1)
				m1 = (m2**r)/((m2**r).sum(axis = 0)[None,:])
				change[j] = np.sum(np.abs(m1 - m2))
			...
			xp.append(xx[ip,:].cpu().detach().numpy())

	xp = np.vstack(xp)

	if verbose == True:
		print('MCL took: %0.2f seconds'%(time() - start_time))
		print('MCL reduced number of sources from %d to %d'%(x.shape[0], xp.shape[0]))
		print('fractional reduction of %0.3f \n'%(xp.shape[0]/x.shape[0]))
...
model_ver = 2
# z = np.load('D:/Projects/Global/Processing/predictions_known_events_real_data_model_ver_%d_date_2022_11_3.npz'%model_ver)
# Out_save = z['Out_save']
# z.close()

# z = np.load('D:/Projects/Global/continuous_time_spatial_grid_points.npz')
# x_query = z['x_query']
# z.close()

z = np.load('D:/Projects/Global/example_continuous_prediction_model_real_data_ver_%d_ver_1.npz'%model_ver)
Out_save = z['Out_save']
x_query = z['x_query']
z.close()
...
srcs_init = np.concatenate((x_query[Out_save[:,0].astype('int')], tsteps_abs[Out_save[:,1].astype('int')].reshape(-1,1), Out_save[:,2].reshape(-1,1)), axis = 1)


## Find break points

## Find split points for comparing catalogs.
total_cuts = 20
t_step_cnt = 2.0 # 2 sec
cnt_radius = 1.0*20.0 # 20 sec
min_seperation = 1.0*20 # 30 min
# n_days = np.ceil(np.maximum(srcs_known_t.max(), Srcs_t.max())/(3600*24)) + 1

tsteps_cnt = np.arange(srcs_init[:,3].min(), srcs_init[:,3].max(), t_step_cnt)
tree1 = cKDTree(srcs_init[:,3].reshape(-1,1))
cnts1 = tree1.query_ball_point(tsteps_cnt.reshape(-1,1), r = cnt_radius)
cnts = np.array([len(cnts1[i]) for i in range(len(cnts1))])

int_median = 5
cnts_trgt = -1.0*(cnts - np.convolve(cnts, np.ones(int_median)/int_median, mode = 'same'))
cnts_trgt[cnts_trgt < 0] = 0.0
cnts_trgt = cnts_trgt/cnts_trgt.sum()

## Find split points
cnt_peaks = cnts - np.convolve(cnts, np.ones(int_median)/int_median, mode = 'same') ## 8 hour moving mean

imins = find_peaks(-1.0*cnt_peaks, height = -1.0*cnt_peaks.mean(), distance = int(min_seperation/t_step_cnt)) ## Note: should add prominence as thresh/2.0, which might help detect nearby events. Also, why is min time spacing set as 2 seconds?
tstep_cuts = np.sort(np.concatenate((np.array([0.0]), tsteps_cnt[imins[0]], np.array([srcs_init[:,3].max() + min_seperation])), axis = 0))

# Count number of picks in time
partitions = kmeans_partition(cnts_trgt, total_cuts, n_batch = 8000, n_steps = 5000, n_sim = 1, lr = 0.001)[0].reshape(-1).astype('int')
partitions = tsteps_cnt[partitions[np.argsort(partitions)]]

tree_mins = cKDTree(tstep_cuts.reshape(-1,1))
tree_mins_matches = tree_mins.query(partitions.reshape(-1,1))[1]

partitions = np.unique(tstep_cuts[tree_mins_matches]) ## These should be the partition points
partitions = np.concatenate((np.array([0.0]), partitions, np.array([srcs_init[:,3].max() + min_seperation])), axis = 0)


## For each partition, first go through and linearly build up edges
## By only checking for matching spatial edges at discrete time steps,
## we can avoid memory overload, as not all temporal and spatial edges
## need to be made at once.

moi

t_win_cluster = 5.0
spc_win_cluster = 75e3

for i in range(len(partitions) - 1):

	i1 = np.where((srcs_init[:,3] < partitions[i + 1])*(srcs_init[:,3] > partitions[i]))[0]

	if len(i1) == 0:
		continue

	srcs = srcs_init[i1]	
	srcs_origin_round = np.round(srcs[:,3])

	t_discretize = np.arange(np.floor(srcs[:,3].min()), np.ceil(srcs[:,3].max()), 1.0)

	tree_t = cKDTree(srcs[:,3].reshape(-1,1))
	lp_t = tree_t.query_ball_point(t_discretize.reshape(-1,1), r = t_win_cluster)

	edges_srcs = []

	for j in range(len(t_discretize)):

		ifind = np.where(srcs_origin_round == t_discretize[j])[0]
		# ifind1 = lp_t[j]

		if len(ifind) > 0:

			for k in range(len(ifind)):

				spc_res = np.linalg.norm(ftrns1(srcs[ifind[k],0:3].reshape(1,-1)) - ftrns1(srcs[lp_t[j],0:3].reshape(-1,3)), axis = 1)

				ifind2 = np.where(spc_res < spc_win_cluster)[0]

				if len(ifind2) > 0:

					edges_srcs.append(np.concatenate((np.array(lp_t[j])[ifind2].reshape(1,-1), ifind[k]*np.ones((1,len(ifind2)))), axis = 0))

		print('%0.4f'%(j/len(t_discretize)))

	edges_srcs = np.hstack(edges_srcs)

	## Instead of making explicit graph, might have to loop over batches of sources, and for each,
	## dynmically create nearest neighbor graph, compute "updates", and mark each source
	## as either being "subsummed" or remaining a local maxima. Continue across all sources, and then
	## until no more updates are made. This way, can locally compute both if a source is "persistent"
	## and the edges for that source, to avoid memory issues and need to compute disconnected components.

	## Slow
	Data_obj = Data(edge_index = to_undirected(torch.Tensor(edges_srcs).long())) # undirected
	nx_g = to_networkx(Data_obj).to_undirected()
	lp = list(nx.connected_components(nx_g))
	clen = [len(list(lp[i])) for i in range(len(lp))]
	## Remove disconnected points with only 1 maxima.
...
# srcs_t = np.unique(np.sort(srcs_init[:,3]))

# tdiff = np.diff(srcs_t)
# ibreak = np.where(tdiff >= break_win)[0]
# srcs_groups_l = []
# ind_inc = 0

# t_break = np.concatenate((np.array([0.0]), srcs_t[ibreak]), axis = 0)

# if len(ibreak) > 0:
# 	for i in range(len(t_break) - 1):
# 		i1 = np.where((srcs_init[:,3] < t_break[i + 1])*(srcs_init[:,3] >= t_break[i]))[0]
# 		srcs_groups_l.append(srcs_init[i1])
# else:
...
for model_ver in [2,3,4,5,6,7]:

	z = np.load('D:/Projects/Global/example_continuous_prediction_model_ver_%d_ver_3.npz'%model_ver)
	Out_save = z['Out_save']
	x_query = z['x_query']
	z.close()
	...
	if (model_ver == 4) or (model_ver == 7):
		...
		if ext_type == 'local':
			...
			z = np.load('/oak/stanford/schools/ees/beroza/imcbrear/Global/example_picks.npz')
		...
		if ext_type == 'local':
			...
			st_glob = glob.glob('D:/Projects/Global/Picks/Copper*picks*ver_1*.npz')

		elif ext_type == 'remote':

			st_glob = glob.glob('/work/wavefront/imcbrear/Global/Picks/Copper*picks*ver_1*.npz')

		elif ext_type == 'server':

			st_glob = glob.glob('/oak/stanford/schools/ees/beroza/imcbrear/Global/Picks/Copper*picks*ver_1*.npz')

		st_glob = list(filter(lambda x: 'all' not in x, st_glob))
		dates = np.vstack([np.array([int(st_glob[i].split('/')[-1].split('_')[1]), int(st_glob[i].split('/')[-1].split('_')[2]), int(st_glob[i].split('/')[-1].split('_')[3])]) for i in range(len(st_glob))])
		t_offset = np.array([UTCDateTime(dates[i,0], dates[i,1], dates[i,2]) - UTCDateTime(2000, 1, 1) for i in range(len(dates))])
		st_glob = [st_glob[i] for i in np.argsort(t_offset)]

		z = np.load(st_glob[chunk_select])

		date = np.array([int(st_glob[chunk_select].split('/')[-1].split('_')[1]), int(st_glob[chunk_select].split('/')[-1].split('_')[2]), int(st_glob[chunk_select].split('/')[-1].split('_')[3])])
		...
		if ext_type == 'local':
			...
			st_glob = glob.glob('D:/Projects/Global/Picks/Copper*all_picks*ver_1*.npz')

		elif ext_type == 'remote':

			st_glob = glob.glob('/work/wavefront/imcbrear/Global/Picks/Copper*all_picks*ver_1*.npz')

		elif ext_type == 'server':

			st_glob = glob.glob('/oak/stanford/schools/ees/beroza/imcbrear/Global/Picks/Copper*all_picks*ver_1*.npz')
	...
	ts = np.arange(-5, 3600*24 + 5, 1)

	srcs_init = np.concatenate((x_query[Out_save[:,0].astype('int'),0:3], ts[Out_save[:,1].astype('int')].reshape(-1,1), Out_save[:,2].reshape(-1,1)), axis = 1)	
	...
	## For each of these run MCL.
	## Possibly add longer range connections,
	## e.g., based on the source - source (with common Cartesian product pathway)

	for k in range(len(srcs_groups_l)):

		srcs = np.copy(srcs_groups_l[k])

		if plot_sources == True:

			## Time trace
			fig, ax = plt.subplots(2,1, figsize = [9.48, 10.0], sharex = True)
			isort = np.argsort(srcs[:,-1])
			ax[0].scatter(srcs[isort,3], srcs[isort,0], c = srcs[isort,-1], alpha = 0.7)
			ax[1].scatter(srcs[isort,3], srcs[isort,1], c = srcs[isort,-1], alpha = 0.7)
			...
			ax[0].set_xlim([srcs[isort,3].min() - 100, srcs[isort,3].max() + 100])

			## Map view
			fig, ax = plt.subplots(1, figsize = [9.48, 5.0], sharex = True)
			isort = np.argsort(srcs[:,-1])
			ax.scatter(srcs[isort,1], srcs[isort,0], c = srcs[isort,-1], alpha = 0.7)
			# ax[1].scatter(srcs[isort,1], srcs[isort,0], c = srcs[isort,-1], alpha = 0.7)
			ax.set_ylabel('Latitude')
			ax.set_xlabel('Longitude')
			# ax.set_xlabel('Time (s)')
			ax.set_ylim([-90, 90])
			ax.set_xlim([-180.0, 180.0])
			ax.scatter(locs[:,1], locs[:,0], s = 5, marker = '^', c = 'r', alpha = 0.5)
