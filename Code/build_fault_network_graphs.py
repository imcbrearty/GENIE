...
if load_clusters == True:
	z = np.load(path_to_file + 'fault_clusters_ver_1.npz')
	clusters = z['clusters']
	z.close()
...
distance_weights = [1.0, 1.5] ## 1 km is 1 km, 1 day is ~10 km
# radius_predict = 10.0 ## 10 km radius to assign events to max magnitudes
radius_predict = 15.0 ## 10 km radius to assign events to max magnitudes
...
def build_input_graph(times, srcs, mags, clusters, scale_pos = 100.0, scale_counts = 100.0, min_trgt_mag = 0.0, vald_win = 365.0, use_vald = False):
	...
	if use_fixed_edges == True:
		...
		k_factor = 2.5
		edges_events = cKDTree(embed_pos).query(embed_pos, k = int(k_factor*knn_edges))[1] # [:,1::]
		k_choose = knn_edges - 1 ## Always choose the self edge
		n_available = edges_events.shape[1]
		edges_events = np.vstack([edges_events[j, np.concatenate((np.array([0]), np.sort(np.random.choice(np.arange(1, n_available), size = k_choose, replace = False))), axis = 0).astype('int')].reshape(1,-1) for j in range(len(edges_events))])
		edges_events = torch.Tensor(np.hstack([np.concatenate((edges_events[j,:].reshape(1,-1), j*np.ones((1,knn_edges))), axis = 0) for j in range(len(embed_pos))])).long().to(device)
	...
	use_zero_depths = True
	if use_zero_depths == True:
		zero_depths = np.array([1.0, 1.0, 0.0]).reshape(1,-1)
	else:
		zero_depths = np.array([1.0, 1.0, 1.0]).reshape(1,-1)



	## Build product pairs
	product_pairs = cKDTree(ftrns1(zero_depths*srcs[ifind,0:3])).query_ball_point(ftrns1(zero_depths*clusters), r = radius_product*1000.0)
	...
	# mag_inpts.clamp_(min = min_trgt_mag)
	cnt_inpts = cnt_inpts/scale_counts

	## Build target max magnitude prediction
	product_pairs_target = cKDTree(ftrns1(zero_depths*srcs[ifind1,0:3])).query_ball_point(ftrns1(zero_depths*clusters), r = radius_predict*1000.0)
	...
	# mag_targets.clamp_(min = min_trgt_mag)
...
class GNN_predict(nn.Module):
	...
	def __init__(self, ftrns1, ftrns2, n_inpt = 10, n_mask = 10, n_hidden = 20, n_embed = 10, device = 'cuda'):
...
ext_save = '/scratch/users/imcbrear/California/FaultTrainingData/'

for j in range(n_batch):

	inpt, embed_pos, embed_clusters, mag_targets, edges_events, edges_clusters, A_prod_events_events, A_prod_clusters_clusters, A_cluster_in_prod, A_cluster_in_event = build_input_graph(times, srcs, mags, clusters, scale_pos = 100.0, scale_counts = 100.0, min_trgt_mag = 0.0)

	z = h5py.File(ext_save + 'training_file_%d.hdf5'%(j + n_batch*argvs[1]), 'w')
	z['inpt'] = inpt.cpu().detach().numpy()
	z['embed_pos'] = embed_pos.cpu().detach().numpy()
	z['embed_clusters'] = embed_clusters.cpu().detach().numpy()
	z['mag_targets'] = mag_targets.cpu().detach().numpy()
	z['edges_events'] = edges_events.cpu().detach().numpy()
	z['edges_clusters'] = edges_clusters.cpu().detach().numpy()
	z['A_prod_events_events'] = A_prod_events_events.cpu().detach().numpy()
	z['A_prod_clusters_clusters'] = A_prod_clusters_clusters.cpu().detach().numpy()
	z['A_cluster_in_prod'] = A_cluster_in_prod.cpu().detach().numpy()
	z['A_cluster_in_event'] = A_cluster_in_event.cpu().detach().numpy()
	z.close()
