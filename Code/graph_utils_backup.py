...
from process_utils import LocalMarching
...
def compute_cdf_analysis(x_grid, ranges, ftrns1_abs, ftrns2_abs):
...
#     def __init__(self, target_N, lat_range, lon_range, depth_range, time_range, 
#                  scale_time_effective=None, use_time_shift=True, use_global=False, 
#                  r_min=None, r_max=None, n_slack_frac=0.1, time_slack_frac=0.1, device='cpu'):
...
#         self.use_global = use_global
#         self.use_time_shift = use_time_shift
#         self.scale_time_effective = scale_time_effective if scale_time_effective is not None else 10000.0
#         self.r_min = r_min
#         self.r_max = r_max
...
#         # 1. Define Base Search Space
#         self.space = [
#             Real(3e3, 15e3, prior='log-uniform', name='scale_t'),
#             Real(0.5, 3.0, name='depth_boost'),
#             Real(1.5, 2.5, name='buffer_scale')
#         ]

#         # 2. Add Slack Dimensions if Enabled
#         if self.n_slack_frac > 0.0:
#             n_min = int(np.round(self.target_N * (1.0 - self.n_slack_frac)))
#             n_max = int(np.round(self.target_N * (1.0 + self.n_slack_frac)))
#             self.space.append(Integer(max(10, n_min), n_max, name='N_nodes'))
#         else:
#             self.space.append(Integer(self.target_N, self.target_N, name='N_nodes'))
...
#             self.space.append(Real(t_min, t_max, name='curr_time_range'))
#         else:
#             self.space.append(Real(self.time_range, self.time_range, name='curr_time_range'))
...
#         @use_named_args(self.space)
#         def objective(scale_t, depth_boost, buffer_scale, N_nodes, curr_time_range):
...
#             # 4. BOUNDARY & DENSITY HEALTH
#             densities = check_boundary_densities(x_grid, self.ranges[0], self.ranges[1], self.ranges[2], curr_time_range, self.use_global)
#             penalty_boundary = (
#                 (1.0 - densities['Depth'])**2 + 
#                 (1.0 - densities['Time'])**2 + 
#                 (1.0 - densities['Lat'])**2 + 
...
#             # 5. CDF ANALYSIS
#             cdf_loss = compute_cdf_analysis(x_grid, self.ranges, self.ftrns1_abs, self.ftrns2_abs)

#             # 6. PENALTY TERMS (REGULARIZATION + SLACK)
#             # Regularize scale_t toward the effective velocity scale
#             reg_scale_penalty = 0.1 * (np.log10(scale_t / self.scale_time_effective) ** 2)

#             # Quadratic Slack Penalty for N
#             n_slack_penalty = w_N_penalty * (((N_nodes - self.target_N) / self.target_N) ** 2) if self.n_slack_frac > 0.0 else 0.0

#             # Quadratic Slack Penalty for time_range
#             t_slack_penalty = w_t_penalty * (((curr_time_range - self.time_range) / self.time_range) ** 2) if self.time_slack_frac > 0.0 else 0.0

#             return cv + (3.0 * cdf_loss) + (0.2 * penalty_boundary) + reg_scale_penalty + n_slack_penalty + t_slack_penalty

#         res = gp_minimize(objective, self.space, n_calls=n_calls, verbose=True)

#         if len(res.x) == 5:
#             best_scale_t, best_depth_boost, best_buffer_scale, best_N, best_time_range = res.x
#         else:
#             best_scale_t, best_depth_boost, best_buffer_scale = res.x[:3]
#             best_N = self.target_N
#             best_time_range = self.time_range
...
#         print(f"Optimal buffer_scale:{best_buffer_scale:.3f}")
#         print(f"Optimal N_nodes:     {best_N} (Target: {self.target_N})")
#         print(f"Optimal time_range:  {best_time_range:.3f} s (Target: {self.time_range:.3f} s)")
#         print(f"Effective Padding:   {buffer_width_phys:.2f} (units)")
...
def perform_ks_depth_test_ellipsoid(x_grid, depth_range, ftrns1_abs):
...
# def soft_component_merger(G, coords, scale_length=None):
#     """
...
#     consistent with the scale-invariant Gaussian kernel framework.
#     """
#     components = sorted(nx.connected_components(G), key=len, reverse=True)
#     if len(components) <= 1:
#         return G
...
#     # Pre-extract local node sigma values if available (fallback to 1.0 if not assigned)
#     # sigma_i is the local density scale factor per node
#     node_sigmas = {
#         n: G.nodes[n].get('sigma', 1.0) 
#         for n in G.nodes()
#     }
...
#         for idx, comp in enumerate(remaining_components):
#             comp_nodes = list(comp)
#             # Find nearest neighbor in main body for each node in candidate component
#             dists, neighbors = main_tree.query(coords[comp_nodes], k=1)
...
#             # --- Scale-Invariant Weight Formula ---
#             # Effective scale length combines global scale with local density multipliers
#             sigma_u = node_sigmas[u]
#             sigma_v = node_sigmas[v]
#             effective_scale = scale_length * np.sqrt(sigma_u * sigma_v)
            
#             # Distance-decay weight safely clamped to (0, 1]
#             w = np.exp(-min_global_dist / max(effective_scale, 1e-8))
...
#     print(f"Soft Merger added {added_bridges} bridge edge(s) using variable-scale boundary logic.")
...
def initialize_sensor_graph_backup(
    ...
    if G is None:
        ...
        # ---------------------------------------------------------
        # Correct Local Scale Setup (Preserves physical distance scaling)
        # ---------------------------------------------------------
        if use_local_scale:
            raw_local_sigmas = compute_local_sigma(coords, k=k_trgt)
            
            # Clip extreme outliers relative to median density (k-NN distance)
            # Allows sparse nodes to scale up, without exploding infinitely
            median_sigma = np.median(raw_local_sigmas)
            bounded_sigmas = np.clip(raw_local_sigmas, 0.3 * median_sigma, 3.0 * median_sigma)
            
            for i in range(n_nodes):
                G.nodes[i]['scale'] = float(bounded_sigmas[i])
        else:
            for i in range(n_nodes):
                G.nodes[i]['scale'] = float(scale_base)
...
def initialize_sensor_graph_backup(
    ...
    # Determine whether we need candidate search matrices
    ...
    if G is None:
        ...
        if use_local_scale:
            raw_local_sigmas = tree.query(coords, k=k_trgt + 1)[0][:, -1]
            median_sigma = np.median(raw_local_sigmas)
            bounded_sigmas = np.clip(raw_local_sigmas, 0.3 * median_sigma, 3.0 * median_sigma)
            for i in range(n_nodes):
                G.nodes[i]['scale'] = float(bounded_sigmas[i])
        ...
        # Apply weights for initial structural edges
        for u, v, d in initial_edges:
            effective_scale = scale_length * (G.nodes[u]['scale'] * G.nodes[v]['scale']) ** (1 / 3)
            w = np.exp(-d / effective_scale)
            if w >= min_weight:
        ...
        # Connect isolated graph components
        G = soft_component_merger(G, coords, scale_length)
        
        # ---------------------------------------------------------
        # OPTIONAL DENSE MATRICES (Only generated if optimizing)
        # ---------------------------------------------------------
    ...
    # 2. SPECTRAL & COMPONENT ANALYSIS
    # ---------------------------------------------------------
    # Fast paths for simple non-optimized initialization on large node sets
    ...
    # Full spectral calculations for optimization passes
    components_list = sorted(nx.connected_components(G), key=len, reverse=True)
    normalize_fiedler = True
...
def initialize_sensor_graph1(
    ...
    if G is None:
        ...
        if set_initial_edges is not None:
            initial_edges = [
                (
                    int(set_initial_edges[0, i]),
                    int(set_initial_edges[1, i]),
                    float(
                        np.linalg.norm(
                            coords[int(set_initial_edges[0, i])]
                            - coords[int(set_initial_edges[1, i])]
                        )
                    ),
                )
                for i in range(set_initial_edges.shape[1])
            ...
            # 3D+ Strategy: k-NN + RNG Check
            if n_dim >= 3:
                k_check = min(int(n_dim * 2), n_nodes - 1)
                distances, indices = tree.query(coords, k=k_check + 1)
                for i in range(n_nodes):
                    p_i = coords[i]
                    for neighbor_idx in range(1, k_check + 1):
                        ...
                        if use_rng:
                            midpoint = (p_i + p_j) / 2.0
                            radius = d_ij - 1e-9
                            potential_violators = tree.query_ball_point(
                                midpoint, radius
                            )
                        ...
                        if i in indices[j, 1 : k_check + 1]:
                            ...
                            initial_edges.append((u, v, float(d_ij)))
            ...
            # 2D Strategy: Gabriel Backbone
                ...
                for simplex in tri.simplices:
                    for i in range(len(simplex)):
                        for j in range(i + 1, len(simplex)):
                            edges_set.add(
                                tuple(sorted((simplex[i], simplex[j])))
                            )
        ...
        if comps:
            ...
            mst_edges = [
                d["dist"]
                for u, v, d in nx.minimum_spanning_tree(
                    lcc, weight="dist"
                ).edges(data=True)
            ]
            scale_length = (
                float(np.maximum(scale_base, np.percentile(mst_edges, 99)))
                if mst_edges
                else float(scale_base)
            )
        else:
            scale_length = float(scale_base)
        ...
        def _compute_edge_weight(d, u_scale, v_scale):
            if use_local_scale:
                eff_scale = (scale_length * u_scale * v_scale) ** (1.0 / 3.0)
            else:
                eff_scale = scale_length
            eff_scale = max(eff_scale, 1e-8)
            return float(np.exp(-d / eff_scale))
        ...
        if init_knn is not None:
            ...
            for j in range(n_nodes):
                for idx_pos, v in enumerate(knn_indices[j, 1:]):
                    d = float(knn_dists[j, idx_pos + 1])
                    w = _compute_edge_weight(
                        d, G.nodes[j]["scale"], G.nodes[v]["scale"]
                    )
                    if w >= min_weight:
                        G.add_edge(
                            int(j),
    ...
    # ---------------------------------------------------------
    # 1D. DENSE MATRICES SYNC (Only when explicitly enabled)
    # ---------------------------------------------------------
    if enable_optimization_matrices:
        ...
        if use_local_scale:
            scale_matrix = (
                scale_length * scales.reshape(-1, 1) * scales.reshape(1, -1)
            ) ** (1.0 / 3.0)
        else:
            scale_matrix = np.full((n_nodes, n_nodes), scale_length)

        G.graph["scale_values"] = scale_matrix

        raw_weights = np.exp(-dists / np.maximum(scale_matrix, 1e-8))
        raw_weights[raw_weights < min_weight] = 0.0
        np.fill_diagonal(raw_weights, 0.0)
    ...
    # ---------------------------------------------------------
    # 2. SPECTRAL & FORMAN-RICCI CURVATURE ANALYSIS
    # ---------------------------------------------------------
    ...
    for comp_id, nodes in enumerate(components_list):
        ...
        if comp_id == 0 and (cnt % 50 == 0):
            G.graph["diameter"] = float(
                nx.algorithms.approximation.diameter(subG)
            )

        # Detect component stability via node set equivalence
        prev_comp_sizes = [
            G.nodes[n].get("comp_size", 0) for n in nodes_sorted
        ]
        current_comp_size = len(nodes_sorted)

        component_is_stable = (
            cnt > 0
            and all(s == current_comp_size for s in prev_comp_sizes)
            and len(set(G.nodes[n].get("comp_id", -1) for n in nodes_sorted))
            == 1
        )

        # Update node metadata for next iteration
        for n in nodes_sorted:
            G.nodes[n]["comp_id"] = comp_id
            G.nodes[n]["comp_size"] = current_comp_size

        if len(nodes_sorted) > 2 and (init_knn is None or init_knn != k_trgt):
            L = nx.laplacian_matrix(
                subG, nodelist=nodes_sorted, weight="weight"
            ).astype(float)

            x_prev = np.array(
                [G.nodes[n].get("fiedler", 0.0) for n in nodes_sorted],
                dtype=float,
            )

            # Warm start ONLY if the component topology remained stable
            if component_is_stable and np.linalg.norm(x_prev) > 1e-5:
                fiedler_vector, fiedler_value, flag = (
                    update_fiedler_warm_start(L, x_start=x_prev)
                )
            else:
                fiedler_vector, fiedler_value, flag = robust_fiedler_solver(L)
            ...
            if comp_id == 0:
                fiedler_graph = float(fiedler_value)
                G.graph["fiedler_value"] = fiedler_graph
            ...
            if ptp > 1e-8:
                fiedler_vector = (
                    2.0 * (fiedler_vector - v_min) / ptp - 1.0
                )
            else:
                fiedler_vector = np.zeros_like(fiedler_vector)
    ...
    # Incremental Forman-Ricci Curvature Pass
    if edges_to_update is None:
        parse_edges = set(G.edges())
        nodes_to_update = set(range(n_nodes))
    else:
        direct_edges = [tuple(sorted((int(e[0]), int(e[1])))) for e in edges_to_update]
        nodes_to_update = set()
        for u, v in direct_edges:
            nodes_to_update.add(u)
            nodes_to_update.add(v)
        
        # Include adjacent edges of touched nodes to maintain exact Ricci calculations
        parse_edges = set(direct_edges)
        for n in nodes_to_update:
            for nbr in G.neighbors(n):
                parse_edges.add(tuple(sorted((n, nbr))))
    ...
    for u, v in parse_edges:
        ...
        sum_u = sum(
            np.sqrt(w_e / max(G[u][n]["weight"], 1e-9))
            for n in G.neighbors(u)
            if n != v
        )
        sum_v = sum(
            np.sqrt(w_e / max(G[v][n]["weight"], 1e-9))
            for n in G.neighbors(v)
            if n != u
        )

        G[u][v]["ricci"] = float(2.0 - sum_u - sum_v)

    # Re-calculate average curvature only for touched nodes
    for u in nodes_to_update:
        vals = [
            G[u][v].get("ricci", 0.0) for v in G.neighbors(u) if "ricci" in G[u][v]
        ]
        G.nodes[u]["total_curvature"] = (
            float(sum(vals) / max(1.0, len(vals))) if vals else 0.0
        )
...
def initialize_sensor_graph(
    ...
    # Centralized edge weight computation formula
    def _compute_edge_weight(d, u_scale, v_scale, scale_base, scale_length):
        if use_local_scale:
            eff_scale = float((scale_base * u_scale * v_scale) ** (1.0 / 3.0))
            eff_scale = max(eff_scale, scale_length * 0.1, 1e-8)
        else:
            eff_scale = scale_length
    ...
    # =========================================================================
    # PHASE 1: ONE-TIME HEAVY INITIALIZATION (Runs ONLY once when G is None)
    # =========================================================================
    if G is None:
        G = nx.Graph()
        G.add_nodes_from(range(n_nodes))
        ...
        # 1. Spatial scales computation (Sparse / KD-Tree based)
        knn_dists_all = tree.query(coords, k=k_query)[0]
        local_scales = np.mean(knn_dists_all[:, 1:], axis=1)
        scale_base = float(np.quantile(knn_dists_all[:, -1], 0.75))

        G.graph["scale_base"] = scale_base
        ...
        # 2. Extract initial topological backbone (RNG for 3D+, Gabriel for 2D)
        initial_edges = []
        if set_initial_edges is not None:
            initial_edges = [
                (
        ...
        # 3. Global scale length estimation from MST
        G_init = nx.Graph()
        G_init.add_weighted_edges_from(initial_edges, weight="dist")
        comps = list(nx.connected_components(G_init))
        if comps:
        ...
        # 4. Add Initial Backbone Edges (Sparse dynamic weight computation)
        for u, v, d in initial_edges:
            w = _compute_edge_weight(
                d,
                G.nodes[u]["scale"],
        ...
        # 5. Add KNN Edges (Sparse dynamic weight computation)
        if init_knn is not None:
            knn_k = min(init_knn + 1, n_nodes)
            knn_dists, knn_indices = tree.query(coords, k=knn_k)
            for j in range(n_nodes):
        ...
        # ---------------------------------------------------------------------
        # 6. OPTIONAL DENSE MATRICES (Only constructed if optimization requires it)
        # ---------------------------------------------------------------------
        if enable_optimization_matrices:
            ...
            if use_local_scale:
                geo_matrix = (
                    scale_base * scales.reshape(-1, 1) * scales.reshape(1, -1)
                ) ** (1.0 / 3.0)
                scale_matrix = np.maximum(geo_matrix, scale_length * 0.1)
            else:
                scale_matrix = np.full((n_nodes, n_nodes), scale_length)

            weights_matrix = np.exp(
                -dists_matrix / np.maximum(scale_matrix, 1e-8)
            )
            weights_matrix = np.clip(weights_matrix, 1e-6, 1.0)
            np.fill_diagonal(weights_matrix, 0.0)

            # Sync explicit graph edge weights (preserving floored backbone weights)
            for u, v, d in G.edges(data=True):
                weights_matrix[u, v] = d["weight"]
                weights_matrix[v, u] = d["weight"]
    ...
    # =========================================================================
        ...
        if edges_to_update is not None and len(edges_to_update) > 0:
            # Fast O(1) Matrix Lookup Path
            if "weights" in G.graph:
                dists_matrix = G.graph["distances"]
                weights_matrix = G.graph["weights"]
                ...
                for u, v in edges_to_update:
                    ...
                    G.add_edge(
                        u,
                        v,
                        dist=float(dists_matrix[u, v]),
                        weight=float(weights_matrix[u, v]),
                        step=cnt,
                    )
            # Dynamic On-Demand Weight Calculation Path (Fallback if dense matrices omitted)
            else:
                scale_base = G.graph.get("scale_base", 1.0)
                scale_length = G.graph.get("scale_length", 1.0)
                ...
                for u, v in edges_to_update:
                    ...
                    G.add_edge(u, v, dist=d, weight=w, step=cnt)
    ...
    for comp_id, nodes in enumerate(components_list):
        ...
        # Robust Component Identification (Tracked via canonical node set freeze)
        node_set = set(nodes_sorted)
        prev_comp_nodes = getattr(G, "_comp_cache", {}).get(comp_id, set())

        # Check topological invariance (same nodes & zero edge modifications)
        has_new_edges = (
            edges_to_update is not None and len(edges_to_update) > 0
        )
        component_is_identical = (
            cnt > 0
            and node_set == prev_comp_nodes
            and not has_new_edges  # Warm-start requires no structural topology changes
        )

        # Cache current topology state
        if not hasattr(G, "_comp_cache"):
            G._comp_cache = {}
        G._comp_cache[comp_id] = node_set

        for n in nodes_sorted:
            G.nodes[n]["comp_id"] = comp_id
            G.nodes[n]["comp_size"] = len(nodes_sorted)
        ...
        if len(nodes_sorted) > 2 and (init_knn is None or init_knn != k_trgt):
            ...
            # Warm-start ONLY if exact component membership and topology are static
            if component_is_identical and np.linalg.norm(x_prev) > 1e-5:
                fiedler_vector, fiedler_value, flag = (
                    update_fiedler_warm_start(L, x_start=x_prev)
                )
            else:
    ...
    # ---------------------------------------------------------
    # INCREMENTAL FORMAN-RICCI CURVATURE (Incident Edge Propagation)
    # ---------------------------------------------------------
    # In initialize_sensor_graph:
    if edges_to_update is None:
        edges_to_evaluate = set(
            tuple(sorted((int(u), int(v)))) for u, v in G.edges()
        )
        nodes_to_update = set(range(n_nodes))
    else:
        # edges_to_update ALREADY contains the 1-hop and 2-hop incident edges
        # generated by get_most_impactful_edges!
        edges_to_evaluate = {
            tuple(sorted((int(e[0]), int(e[1])))) for e in edges_to_update
        }
        nodes_to_update = set()
    ...
    # Re-calculate node average total curvature for modified neighborhoods
    for u in nodes_to_update:
        vals = [
            G[u][v]["ricci"] for v in G.neighbors(u) if "ricci" in G[u][v]
        ]
        G.nodes[u]["total_curvature"] = (
            float(sum(vals) / max(1.0, len(vals))) if vals else 0.0
        )
...
#     # Prevent scale from collapsing below 0.5 * global scale_length
#     geo_scale = np.sqrt(scale_u * scale_v)
#     effective_scale = max(scale_length * geo_scale, scale_length * 0.5, 1e-4)
    
#     # Exponent argument limit: -np.log(min_weight) ensures w >= min_weight
#     scaled_dist = d / effective_scale
#     w = np.exp(-scaled_dist)
...
def optimize_station_graph(locs_use, ftrns1, k_sta_edges, init_knn = 3, max_iters = 5000, compute_stats = True):
...
def optimize_source_graph(x_grid, ftrns1, k_spc_edges, scale_time, k_init_ratio = 0.8, max_iters = 5000, compute_stats = True):
    k_trgt = k_spc_edges
...
def compute_graph_stats(G, cnt = 0):
    ...
    def build_final_subgraph(self, target_node_count=100000, anchor_fraction = 0.15, slack_factor = 2.5, skip_paths = False):
        ...
        if not skip_paths:
            ...
            for i in range(len(sorted_anchors) - 1):
                ...
                # OPTIMIZED: Clean single-pass batch injection without inner-loop comprehensions
                nodes_to_retain.update((na, u[1]) for na in path_a)
                for na in path_a:
                    count_A[na] += 1
                    count_B[u[1]] += 1
        ...
        # OPTIMIZED: Chunk-based vector pre-sampling to prevent single-iteration O(N) calls
        chunk_size = 5000
        chunk_idx = chunk_size  # Trigger generation immediately on loop start
        ...
        # OPTIMIZED: Factored index mapping to transform global scans into local lookups
        retained_by_A = {}
        retained_by_B = {}
        for (a, b) in nodes_to_retain:
...
class SpectralProductSampler_main_backup:
    ...
    def build_final_subgraph(self, target_node_count=100000, anchor_fraction = 0.15, slack_factor = 2.5, skip_paths = False):
        ...
        if not skip_paths:
            ...
            # PROTECT BLOBS: Cap structural backbone paths at 40% of target budget
            skeleton_cap = int(target_node_count * 0.40)
        ...
        for a, b in list(nodes_to_retain):
            ...
            if count_A[a] < MAX_DEG_A:
                for nb in self.G_B.neighbors(b):
                    if (a, nb) not in nodes_to_retain:
                        # FIXED: Correctly added parent 'b' to provide exactly 4 items for unpacking
                        neighbor_queue.append((a, b, nb, 'B'))
...
class SpectralProductSampler_updated1:
    def __init__(self, G_A, G_B, pos_A, pos_B, sparse_threshold=2000, k_approx=150):
        self.G_A = G_A.copy()
        self.G_B = G_B.copy()
        self.pos_A = pos_A
...
class SpectralProductSampler_updated:
    ...
    def select_anchors_backup(
        ...
        # ------------------------------------------------------------------
        # Step B: Per-Source Adaptive Physical Sampling (Vectorized)
        # ------------------------------------------------------------------
        n_physical_target = int(n_anchor_target * physical_ratio)
        ...
        if n_physical_target > 0:
            ...
            # Dynamic calculation of K per source derived from physical_ratio
            k_per_source_exact = n_physical_target / max(1, n_a)
            k_draw_base = int(np.floor(k_per_source_exact))
            prob_extra = k_per_source_exact - k_draw_base
            ...
            row_sums = equalized_weights.sum(axis=1, keepdims=True)
            # Normalize per row; fallback to uniform if a row sum is zero
            p_matrix = np.where(row_sums > 0, equalized_weights / np.maximum(row_sums, 1e-12), 1.0 / k_actual_B)
            # Apply 10% uniform exploration floor across the candidate set
            p_matrix = 0.9 * p_matrix + (0.1 / k_actual_B)
            p_matrix /= p_matrix.sum(axis=1, keepdims=True)
    
            # --- VECTORIZED NUMBER OF DRAWS PER SOURCE ---
            extra_draws = (np.random.rand(n_a) < prob_extra).astype(int)
            n_draws_per_source = np.clip(k_draw_base + extra_draws, 1, k_actual_B)
            max_draws = n_draws_per_source.max()
    
            # --- VECTORIZED SAMPLING WITHOUT REPLACEMENT (Gumbel-Max trick) ---
            # Draw Gumbel noise across all candidate entries simultaneously
            u = np.random.uniform(1e-10, 1.0 - 1e-10, size=(n_a, k_actual_B))
            gumbel_scores = np.log(p_matrix) - np.log(-np.log(u))
            
            # Ranking candidate columns by score gives exact categorical samples without replacement
            top_col_indices = np.argsort(-gumbel_scores, axis=1)[:, :max_draws]
    
            # Extract sampled station indices and construct anchor pairs
            row_indices, rank_indices = np.ogrid[:n_a, :max_draws]
            valid_mask = rank_indices < n_draws_per_source[:, None]
    
            sampled_source_rows = np.repeat(np.arange(n_a)[:, None], max_draws, axis=1)[valid_mask]
            sampled_cand_cols = top_col_indices[valid_mask]
            sampled_station_cols = indices_B[sampled_source_rows, sampled_cand_cols]
    
            for r_idx, sta_idx in zip(sampled_source_rows, sampled_station_cols):
                anchors.add((self.nodes_A[r_idx], self.nodes_B[sta_idx]))
    ...
    def expand_partial_stars_backup(
        ...
        for a, b in shuffled_anchors:
            ...
            for factor in factors:
                ...
                if factor == 'A':
                    # CORRECTED: Properly slice sparse CSR neighborhood
                    neigh_indices = indices_A[indptr_A[idx_a]:indptr_A[idx_a + 1]]
                    unvisited = [n_idx for n_idx in neigh_indices if (self.nodes_A[n_idx], b) not in retained_nodes]
...
class SpectralProductSampler:
    def __init__(self, G_A, G_B, pos_A, pos_B, sparse_threshold=2000, k_approx=100):
        ...
        # after _compute_spectral_stats and _precompute_edge_weights
        str_A = self._get_strengths(self.G_A, self.nodes_A)
        str_B = self._get_strengths(self.G_B, self.nodes_B)

        eps_str = 0.5
        self.tau_A = self.tau_A / (str_A + eps_str)
        self.tau_B = self.tau_B / (str_B + eps_str)

        self.tau_A = np.maximum(self.tau_A, 1e-12)
        self.tau_B = np.maximum(self.tau_B, 1e-12)

        sum_tau_A = self.tau_A.sum()
        sum_tau_B = self.tau_B.sum()
        self.p_A = self.tau_A / sum_tau_A if sum_tau_A > 0 else np.full(len(self.tau_A), 1.0 / len(self.tau_A))
        self.p_B = self.tau_B / sum_tau_B if sum_tau_B > 0 else np.full(len(self.tau_B), 1.0 / len(self.tau_B))

        self.mean_tau_A = float(self.tau_A.mean())
        self.mean_tau_B = float(self.tau_B.mean())

    def _compute_spectral_stats(self, G, nodelist, mode):
        L = nx.laplacian_matrix(G, nodelist=nodelist, weight='weight').astype(np.float64)
        n = L.shape[0]
    ...
    def _get_strengths(self, G, nodelist):
        """Return array of weighted degrees (strengths) aligned with nodelist."""
        # NetworkX 3.x
        try:
            return np.array([G.degree(n, weight='weight') for n in nodelist], dtype=np.float64)
        except TypeError:
            # older NetworkX
            return np.array([sum(d.get('weight', 1.0) for _, _, d in G.edges(n, data=True))
                             for n in nodelist], dtype=np.float64)
    ...
    def _precompute_edge_weights(self):
        for G, L_obj, mode, mapping, nodelist in [
            ...
            if mode == 'dense':
                # already correct – uses the pseudoinverse which saw the weights
                diag = np.diag(L_obj)
                for u, v in G.edges():
                    u_idx, v_idx = mapping[u], mapping[v]
                    w = max(1e-6, diag[u_idx] + diag[v_idx] - 2.0 * L_obj[u_idx, v_idx])
                ...
                # sparse approximation that respects weights
                str_map = dict(zip(nodelist, self._get_strengths(G, nodelist)))
                for u, v, d in G.edges(data=True):
                    # optional: also incorporate the original edge weight
                    orig_w = d.get('weight', 1.0)
                    w = orig_w / np.sqrt(max(1e-6, str_map[u] * str_map[v]))
                    G[u][v]['res_w'] = float(max(1e-6, w))
    ...
    def derive_adaptive_sampling_params(self, k_local=10):
        ...
        avg_str_A = np.mean(self._get_strengths(self.G_A, self.nodes_A)) if len(self.G_A) else 1.0
        avg_str_B = np.mean(self._get_strengths(self.G_B, self.nodes_B)) if len(self.G_B) else 1.0
        avg_deg = (avg_str_A + avg_str_B) / 2.0
    ...
    def select_anchors(
        ...
        min_spectral_frac = 0.15 if self.aspect < 10 else 0.08
        n_spectral_target = max(
        ...
        if n_remaining > 0:
            ...
            sampled_b = np.random.choice(n_b, size=n_remaining * 2, replace=True, p=self.p_B)
            for idx_a, idx_b in zip(sampled_a, sampled_b):
                pair = (self.nodes_A[idx_a], self.nodes_B[idx_b])
                anchors.add(pair)
                spectral_anchors.add(pair)
                if len(anchors) >= n_anchor_target:
                    break

        only_spectral = spectral_anchors - existing_anchors   # pairs that are *new* and came from spectral

        anchor_list = list(anchors)
        random.shuffle(anchor_list)

        # build mask in the *shuffled* order
        spectral_mask = np.array([1 if pair in only_spectral else 0 for pair in anchor_list], dtype=np.int8)
        return anchor_list, spectral_mask
    ...
    def expand_partial_stars_backup(
        ...
        shuffled_anchors = np.array(list(anchors), dtype=object)    

        # Generate a random permutation of indices
        p = np.random.permutation(len(shuffled_anchors))    

        # Reorder both structures using the exact same permutation
        shuffled_anchors = shuffled_anchors[p].tolist()
        spectral_mask = spectral_mask[p]
    ...
    def expand_partial_stars(
        ...
        for (a, b), mask in zip(shuffled_anchors, spectral_mask):
            ...
            is_spectral = (mask == 1)
            can_expand = bool(unvisited_A or unvisited_B)
            over_budget = len(retained_nodes) >= soft_budget_cap
        
            # Physical: always keep
            # Spectral: keep only if we can still expand it
            if is_spectral and (over_budget or not can_expand):
                continue
            ...
            # If over budget we already skipped spectral; physical anchors stop here
            if over_budget:
                continue
        
            # Expand (only reached when under budget)
            mult_a = np.clip(self.tau_A[idx_a] / self.mean_tau_A, 0.5, 3.0)
            mult_b = np.clip(self.tau_B[idx_b] / self.mean_tau_B, 0.5, 3.0)
            mult_a *= 2.5 if is_spectral else 1.0
            mult_b *= 2.5 if is_spectral else 1.0
    ...
    # def estimate_anchor_target(self, target_node_count, k_base=None, spatial_coherence_eta=0.7):
    #     """
    #     Principled estimate of required anchor count using occupied volume statistics 
    #     accounting for spatial overlap without arbitrary magic constants.
    #     """
    #     if k_base is None:
    #         derived = self.derive_adaptive_sampling_params()
    #         k_base = derived["k_base"]
    ...
    #     # 1. Expected 1-hop partial star footprint per anchor
    #     ratio_a = np.sqrt(n_a / n_b)
    #     ratio_b = np.sqrt(n_b / n_a)
    #     star_size = 1.0 + k_base * (ratio_a + ratio_b)
    
    #     # 2. Coverage fraction in total space
    #     target_ratio = min(0.999, target_node_count / total_space)
        
    #     # 3. Solved occupancy formula for effective draws required under spatial clustering
    #     effective_draws = -np.log(1.0 - target_ratio) * total_space
        
    #     # 4. Convert effective draws to anchor count adjusting for spatial coherence
    #     raw_anchors = effective_draws / (star_size * max(0.1, spatial_coherence_eta))
        
    #     # Upper bound physical anchors by available nodes or target budget
    #     max_anchors = min(total_space, target_node_count)
    ...
    def subgraph_diagnostics(self, retained_nodes, G_sub=None):
        if G_sub is None:
            G_sub = self.build_networkx_subgraph(retained_nodes)
        degrees = np.array([d for _, d in G_sub.degree()])
        n = len(retained_nodes)
        isolates = int((degrees == 0).sum())
        # coverage of factors
        used_A = len({a for a, _ in retained_nodes})
        used_B = len({b for _, b in retained_nodes})
        return {
            "n_nodes": n,
            "n_edges": G_sub.number_of_edges(),
            "frac_isolates": isolates / max(1, n),
            "mean_deg": float(degrees.mean()) if n else 0.0,
            "max_deg": int(degrees.max()) if n else 0,
            "coverage_A": used_A / len(self.nodes_A),
            "coverage_B": used_B / len(self.nodes_B),
    ...
    def sample_subgraph(
        ...
        while len(retained_nodes) < target_node_count and pass_idx < max_interleaved_passes:
            ...
            anchors_batch, spectral_mask = self.select_anchors(
                n_anchor_target=n_anchor_target,
            ...
            # 1. Filter out COMMITTED anchors while keeping the mask in sync
            fresh_pairs = [
                (a, mask) for a, mask in zip(anchors_batch, spectral_mask) 
                if a not in all_sampled_anchors
            ]        

            if fresh_pairs:
                # Unpack into separate filtered lists
                fresh_anchors, fresh_spectral_mask = zip(*fresh_pairs)
                fresh_anchors = list(fresh_anchors)
                fresh_spectral_mask = np.array(fresh_spectral_mask)
            else:
                # 2. Fallback: draw purely random unvisited product nodes
                ...
                # Fallback anchors are not spectral leverage anchors, so mark them as 0s
                fresh_spectral_mask = np.zeros(len(fresh_anchors), dtype=int)
...
#             depth_boost=depth_boost, ## Should use depth boost (check Hawaii case)
...
#         ## Project domain scale parameters
#         x_grid_cart = ftrns1(x_grid)
#         x_grid_proj = np.concatenate((x_grid_cart, scale_time*x_grid[:,[3]]), axis = 1)
...
# def fit_spatial_domain(locs_use, stas_use, scale_domain, deg_padding, number_of_spatial_nodes, k_spc_edges, k_sta_edges, depth_range, ftrns1, ftrns2, use_global = False, max_nodes = 3000, n_trgt_nodes = 200e3, Vc = 3500.0, file_index = 0, date = [2000, 1, 1], rbest = None, mn = None, domain = None, n_rand_srcs = 150, quantile_times = 0.35, quantile_times_srcs = 0.5, use_tuner = True, n_tuner_steps = 50, verbose = True, device = 'cpu'):
...
#                                          scan_step_m=domain_scale['W_phys_m']/2.0, W_phys_m = domain_scale['W_phys_m'], W_t = domain_scale['W_t_s'], r_min = r_min, r_max = r_max, device=device)
...
def build_graphs_domain(m_domain, locs_use, stas_use, scale_domain, deg_padding, number_of_spatial_nodes, k_spc_edges, k_sta_edges, depth_range, ftrns1, ftrns2, use_global = False, assign_based_on_grid = False, max_nodes = 3000, n_trgt_nodes = 200e3, Vc = 6500.0, max_time_shift_range = None, file_index = 0, date = [2000, 1, 1], n_grids = 1, initialize = None, fixed_domain = None, use_paths = False, rbest = None, mn = None, optimize_station_graphs = False, optimize_source_graphs = False, use_domain_approximate = False, use_tuner = True, n_tuner_steps = 50, name_of_project = '', verbose = True, device = 'cpu'):
    ...
    if fixed_domain is not None:
        domain['lat_range'] = fixed_domain[0]
        domain['lon_range'] = fixed_domain[1]
    ...
    # A_prod_sta_sta_l = []
    # A_prod_src_src_l = []
    # A_prod_weights_sta_sta_l = []
    # A_prod_weights_src_src_l = []
    # Ac_l = []
    ...
    for n_grid in range(num_grids):


        ## Project domain scale parameters
        x_grid = np.copy(x_grids[n_grid])
        x_grid_cart = ftrns1(x_grid)
        x_grid_proj = np.concatenate((x_grid_cart, scale_time*x_grid[:,[3]]), axis = 1)
        x_grid_carts.append(np.expand_dims(x_grid_cart, axis = 0))
        x_grid_projs.append(np.expand_dims(x_grid_proj, axis = 0))
        ...
        if initialize is not None:
            data_save['A_src_in_sta_%d'%n_grid] = A_src_in_sta
            data_save['A_sta_%d'%n_grid] = A_sta
            data_save['A_src_%d'%n_grid] = A_src
            data_save['A_prod_sta_sta_%d'%n_grid] = A_prod_sta_sta
            data_save['A_prod_src_src_%d'%n_grid] = A_prod_src_src
    ...
    if initialize is not None:
        ...
        data_save['scale_time'] = scale_time
        data_save['depth_boost'] = depth_boost
        data_save['locs_use'] = locs_use
        data_save['stas_use'] = stas_use
        # data_save['srcs_cart'] = x_grid_cart
        data_save['locs_cart'] = locs_cart
        data_save['lat_range'] = lat_range
        data_save['lon_range'] = lon_range
        data_save['lat_range_extend'] = lat_range_extend
        data_save['lon_range_extend'] = lon_range_extend
        data_save['depth_range'] = depth_range
        data_save['deg_padding'] = deg_padding
        data_save['time_shift_range'] = time_shift_range
        data_save['source_label_width'] = source_label_width
        data_save['source_label_width_t'] = source_label_width_t
        data_save['association_label_width'] = association_label_width
        data_save['association_label_width_t'] = association_label_width_t
        data_save['sigma_input'] = sigma_input
        data_save['ichoose_grid'] = 0
        data_save['rbest'] = rbest
        data_save['mn'] = mn

        x_grid_carts = np.vstack(x_grid_carts)
        x_grid_projs = np.vstack(x_grid_projs)
        data_save['x_grids'] = x_grids
        data_save['x_grid_carts'] = x_grid_carts
...
def fit_spatial_domain(locs_use, stas_use, scale_domain, deg_padding, number_of_spatial_nodes, k_spc_edges, k_sta_edges, depth_range, ftrns1, ftrns2, use_global = False, max_nodes = 3000, n_trgt_nodes = 200e3, Vc = 6500.0, file_index = 0, date = [2000, 1, 1], rbest = None, mn = None, domain = None, initialize = None, fixed_domain = None, max_time_shift_range = None, min_time_range = 5.0, n_rand_srcs = 250, quantile_times = 0.4, quantile_times_srcs = 0.4, extend_ratio = 2.0, use_tuner = True, n_grids = 1, n_tuner_steps = 50, verbose = True, device = 'cpu'):
    ...
    # Unpack using your exact variable names
    Volume, Volume_space, Area, nominal_spacing, nominal_spacing_space, nominal_spacing_time = metrics
    if initialize is None: n_grids = 1
...
def get_most_impactful_edges_balanced_backup(G, scale_length, cnt = 0, top_k=5, degree_p=1.0, greedy_regularize = 0.9, greedy_regularize_extra = 0.1, alpha_scale = 0.75, beta_scale = 0.1, min_weight = 0.05, exclusion_scale = 1.0, mode = 'univariate', use_triangles = False, update = False): # tracked_values = None, track_resistance = True
...
# def estimate_kernel_widths3(domain, station_locs, z_range = (-40000, 2000), Vs = 3500.0, noise_level = 0.02, n_srcs = 250, n_test_per_src = 10000, n_neighbors_trgt = 20, use_global = False, device = 'cpu'):
#     import torch
#     import numpy as np
#     from scipy.spatial.distance import cdist
...
#     # --- FIXED: Vectorized Ellipsoidal Surface Distance Helper ---
#     def trv(locs, srcs, Vs = 3500.0):
#         # locs: (N_stas, 3), srcs: (S, 3) in LLA
#         # Convert LLA arrays cleanly to ECEF tensors
...
#         # 1. WGS84 Constants for Radius calculation
#         a, b = 6378137.0, 6356752.3142
        
#         # 2. Extract unit vectors from Earth's center
#         locs_norm = locs_ecef / torch.norm(locs_ecef, dim=1, keepdim=True)
#         srcs_norm = srcs_ecef / torch.norm(srcs_ecef, dim=1, keepdim=True)
        
#         # 3. Compute local ellipsoidal radii using geocentric latitude approximation
...
#         r_avg = 0.5 * (r_locs.unsqueeze(0) + r_srcs.unsqueeze(1)) # (S, N_stas)
        
#         # 4. Angular separation via dot product matrix multiplication
#         cos_theta = torch.clamp(torch.mm(srcs_norm, locs_norm.T), -1.0, 1.0)
#         theta = torch.acos(cos_theta) # (S, N_stas)
        
#         # 5. True ellipsoidal surface distance travel times
#         t_ellipsoid = (theta * r_avg) / Vs
#         return t_ellipsoid.unsqueeze(2) # Return shape matching original output (S, N_stas, 1)
...
#     for s in range(n_srcs):
#         s_stas = station_locs[nearest_idx[s]]
#         t_r = trv(torch.Tensor(s_stas).to(device), torch.Tensor(src_refs_lla[s:s+1]).to(device), Vs=Vs).cpu().detach().numpy()
#         t_obs_list.append(t_r[:, :, 0] + np.random.normal(0, t_r[:, :, 0] * noise_level))
        
#         t_t = trv(torch.Tensor(s_stas).to(device), torch.Tensor(test_lla_flat[s*n_test_per_src : (s+1)*n_test_per_src]).to(device), Vs=Vs).cpu().detach().numpy()
#         t_test_list.append(t_t[:, :, 0])

#     t_obs = np.stack(t_obs_list, axis=0)  # (S, 1, 20) -> adjust to squeeze out single batch item securely
#     t_test = np.stack(t_test_list, axis=0) 

#     # 5. Vectorized Chi-Square Misfit
#     residuals = t_obs[:, :, None, :] - t_test[:, None, :, :]
#     residuals = residuals.squeeze(1) - time_offs[:, :, None] # Adjust dims smoothly
#     sigma_d = np.maximum(t_test * noise_level, 1e-6)
#     chi_error = np.sqrt(np.mean((residuals / sigma_d)**2, axis=2)) 

#     # 6. Extract Widths
#     dist_s = np.linalg.norm(space_offs, axis=2) 
#     dist_t = np.abs(time_offs) 
...
# def estimate_kernel_widths4(domain, station_locs, z_range = (-40000, 2000), Vs = 3500.0, noise_level = 0.02, n_srcs = 250, n_test_per_src = 10000, n_neighbors_trgt = 20, use_global = False, device = 'cpu'):
#     import torch
#     import numpy as np
#     from scipy.spatial.distance import cdist
...
#     # # Vectorized Ellipsoidal Surface Distance Helper
#     # def trv(locs, srcs, Vs=3500.0):
#     #     locs_ecef = torch.tensor(lla2ecef(locs.cpu().detach().numpy()), dtype=torch.float32, device=device)
#     #     srcs_ecef = torch.tensor(lla2ecef(srcs.cpu().detach().numpy()), dtype=torch.float32, device=device)
...
#     def trv(locs, srcs, Vs=3500.0):
#         # Convert LLA input arrays cleanly to ECEF PyTorch tensors
#         locs_ecef = torch.tensor(lla2ecef(locs.cpu().detach().numpy()), dtype=torch.float32, device=device)
#         srcs_ecef = torch.tensor(lla2ecef(srcs.cpu().detach().numpy()), dtype=torch.float32, device=device)
        
#         # Determine tracking footprint size dynamically for this specific batch
#         # We sample a temporary baseline distance matrix to find the local aperture scale
#         sample_dists = torch.cdist(locs_ecef, locs_ecef)
#         batch_aperture = torch.max(sample_dists).item()

#         if batch_aperture < 300000.0:
#             # Local/Regional Mode (< 300 km): True 3D Euclidean space to preserve depth (Z) sensitivity
#             # Shape: (S, N_stas) -> unsqueeze to match original (S, N_stas, 1) layout
#             t_euclidean = torch.cdist(srcs_ecef, locs_ecef) / Vs
#             return t_euclidean.unsqueeze(2)
#         else:
#             # Global Mode (>= 300 km): True WGS84 Ellipsoidal Surface Arc Distances
#             a, b = 6378137.0, 6356752.3142
#             locs_norm = locs_ecef / torch.norm(locs_ecef, dim=1, keepdim=True)
#             srcs_norm = srcs_ecef / torch.norm(srcs_ecef, dim=1, keepdim=True)
...
#         # --- MATCHING ADAPTIVE VELOCITY LOGIC ---
...
#         # # --- UPDATED: Adaptive S-Wave Baseline for Mixed-Phase Processing ---
#         # if local_aperture < 300000.0:    # Regional/Local Crustal (< 300 km)
#         #     vel_avg = 3500.0             # Typical crustal Vs
#         #     vel_min = 2500.0
#         # elif local_aperture < 1500000.0:  # Large Continental/Upper Mantle (< 1500 km)
#         #     vel_avg = 4700.0             # Upper mantle Vs (e.g., Lithospheric mantle)
#         #     vel_min = 4000.0
#         # else:                            # Global/Deep Mantle Teleseismic
#         #     vel_avg = 6500.0             # Average deep mantle Vs (climbing toward CMB)
#         #     vel_min = 5000.0
...
#         srcs_maxima = mp(srcs_init.cpu().detach().numpy(), ftrns1, tc_win=W_t, sp_win=(1.5*W_phys_m), scale_depth=0.2, n_steps_max=2)
...
def probe_network_sidelobes_geodetic(
    ...
    # 2. P-Wave Adaptive Velocity Scaling
    override_vc = True
    if override_vc:
    ...
    # 3. Generate Ground-Truth Target & Candidate Grid
    src_true = np.array([
    ...
    # 5. Coherence Mapping & Main Peak Gate
    W_t_scalar = 1.0
    rel_threshold = k_stations * 0.15                                           
    mismatch = torch.abs((t_obs.unsqueeze(0) - t_calc) - cand_dt.unsqueeze(1))
    coherence = torch.exp(-mismatch / (W_t_scalar * W_t)).sum(dim=1)
    ...
    # Matching search radius with suppression core (1.5x W_phys_m)
    ...
    # 6. Peak Suppression via Local Marching
    if len(srcs_init) > 0:
        mp = LocalMarching(device=device)
...
def probe_network_sidelobes_geodetic_backup(station_latlonz, domain_lat_range, domain_lon_range, domain_depth_range, ftrns1, ftrns2,
    ...
    # --- 1. Project Stations & Determine Search Scale ---
    scale_max = 1.2
    station_xyz = torch.tensor(ftrns1(station_latlonz), device=device, dtype=torch.float32)
    dist_matrix = torch.cdist(station_xyz, station_xyz)
    d_array = scale_max*torch.max(dist_matrix).item()
    ...
    # --- 3. Generate Sparse 4D Point Cloud ---
    # We use your regular_sobolov to sample (Lat, Lon, Depth, Time) simultaneously
    # This replaces the meshgrid + dt_range loops
    # num_candidates = 50000 # Adjust based on complexity vs speed trade-off
    
    # Matching your provided function call structure
    trial_points, _ = regular_sobolov(
        num_candidates, 
        lat_range=domain_lat_range, 
        lon_range=domain_lon_range, 
        depth_range=domain_depth_range, 
        time_range=max_dt, 
        use_time=True, 
        use_global=use_global, 
        scale_time=vel_avg, # Often used to isotropize time into distance units ## Should likely use isotropic values here, and for the query points?
        N_target=num_candidates,      # Target spatial density
        buffer_scale=0.0, 
        r_min=r_min, 
        r_max=r_max
    )
    ...
    # trial_points: [N, 4] -> (Lat, Lon, Depth, dt)

    # --- 4. Vectorized Coherence Screening ---
    # Get K-nearest stations to truth for pick generation
    all_dists = torch.cdist(src_true_xyz, station_xyz).squeeze(0)
    _, sta_idx = torch.topk(all_dists, k=k_stations, largest=False)
    active_stas_xyz = station_xyz[sta_idx]
    ...
    # Generate "True" picks
    t_obs = torch.norm(active_stas_xyz - src_true_xyz, dim=1) / vel_avg
    t_obs_picks = np.concatenate((
        t_obs.cpu().numpy().reshape(-1, 1), 
        sta_idx.cpu().numpy().reshape(-1, 1)
    ), axis=1)


    W_t_scalar = 1.0
    mismatch_true = torch.abs(t_obs - (torch.norm(active_stas_xyz - src_true_xyz, dim=1) / vel_avg))
    max_possible_coherence = torch.exp(-mismatch_true / (W_t_scalar*W_t)).sum().item()
    rel_threshold = max_possible_coherence * 0.15


    # Project all trial points to XYZ for travel time calculation
    cand_xyz = torch.tensor(ftrns1(trial_points[:, :3].cpu().numpy()), device=device, dtype=torch.float32)
    cand_dt = trial_points[:, 3]

    # Batch compute travel times [N_candidates, K_stations]
    t_calc = torch.cdist(cand_xyz, active_stas_xyz) / vel_avg
    
    # Coherence: Shift calculated times by the sampled dt
    # mismatch: [N, K]
    mismatch = torch.abs((t_obs.unsqueeze(0) - t_calc) - cand_dt.unsqueeze(1))
    coherence = torch.exp(-mismatch / (W_t_scalar*W_t)).sum(dim=1)

    # --- 5. Initial 4D Non-Maximum Suppression (Sparse) ---
    w_t_m = W_t * vel_avg 
    suppress_r_4d = torch.sqrt(torch.tensor(W_phys_m**2 + w_t_m**2)) * 1.5

    # Filter Main Peak immediately
    d_space_true = torch.norm(cand_xyz - src_true_xyz, dim=1)
    d_time_true = torch.abs(cand_dt - 0.0) * vel_avg
    dist_4d_from_true = torch.sqrt(d_space_true**2 + d_time_true**2)
    
    valid_mask = (dist_4d_from_true > suppress_r_4d) & (coherence > rel_threshold)
    candidate_indices = torch.where(valid_mask)[0]
    srcs_init = torch.cat((trial_points[candidate_indices], coherence[candidate_indices].reshape(-1,1)), dim = 1)
    peaks = []
    ...
    if len(srcs_init) > 0:
        ...
        mp = LocalMarching(device=device)
        # Marching typically refines coordinates and can return coherence values
        srcs_maxima = mp(
            srcs_init.cpu().detach().numpy(), 
            ftrns1, 
            tc_win=W_t, 
            sp_win=W_phys_m, 
            scale_depth=0.2, 
        ...
        # --- 6. Local Marching Refinement ---
        if len(srcs_maxima) > 0:

            # Convert refined points back to your requested output format
            for i in range(len(srcs_maxima)):
                p_coord = srcs_maxima[i] # [lat, lon, z, dt]
                p_xyz = torch.tensor(ftrns1(p_coord[:3].cpu().numpy().reshape(1,-1)), device=device).reshape(-1)
                
                # Recalculate final metrics for the peak
                d_s_final = torch.norm(p_xyz - src_true_xyz)
                d_t_final = torch.abs(p_coord[3] - 0.0) * vel_avg
                
                peaks.append({
                    'pos': p_xyz,
                    'pos_src': p_coord[:3].cpu().numpy(),
                    'val': 0.0, # You can re-run coherence calculation here if needed
                    'dt_offset': p_coord[3].item(),
                    'dist_offset_m': d_s_final.item(),
                    'dist_4d_m': torch.sqrt(d_s_final**2 + d_t_final**2).item()
                })
...
#     # --- UPDATE: Base search bounds on the ACTIVE cluster's footprint ---
#     # Sample a quick baseline true source at the center of your domain to find its station neighborhood
#     src_true_init = np.array([
#         np.mean(domain_lat_range),
#         np.mean(domain_lon_range),
#         np.mean(domain_depth_range)
#     ]).reshape(1, -1)
#     src_true_init_xyz = torch.tensor(ftrns1(src_true_init), device=device, dtype=torch.float32)
...
#     # Maximum physical aperture across just the active k_stations tracking cluster
#     active_cluster_dists = torch.cdist(station_xyz[sta_idx_init], station_xyz[sta_idx_init])
#     local_aperture = torch.max(active_cluster_dists).item()
    
#     # Scales bounds nicely: matches full network on local maps; stays constrained on massive/global maps
#     d_array = scale_max * local_aperture
#     max_radius_m = d_array / 2.0
#     max_dt = d_array / vel_min
...
#     # --- 5. UPDATE: Box-Intersection Non-Maximum Suppression Filter ---
#     d_space_true = torch.norm(cand_xyz - src_true_xyz, dim=1)
#     d_time_true = torch.abs(cand_dt - 0.0)
    
#     # Establish independent resolution boundaries around the main pulse
#     is_inside_spatial_core = d_space_true <= (W_phys_m * 1.5)
#     is_inside_temporal_core = d_time_true <= (W_t * 1.5)
    
#     # Suppress ONLY if a point is trapped inside BOTH boundaries simultaneously.
#     # This leaves the time axis wide open to catch fixed-location temporal ghosts.
#     is_main_peak = is_inside_spatial_core & is_inside_temporal_core
#     valid_mask = (~is_main_peak) & (coherence > rel_threshold)
...
# def probe_network_sidelobes_geodetic3(station_latlonz, domain_lat_range, domain_lon_range, domain_depth_range, ftrns1, ftrns2,
#                                      k_stations=20, vel_avg=3500.0, vel_min=2500.0,
#                                      scan_step_m=1000.0, W_phys_m=1000.0, W_t=3.0, r_min = None, r_max = None, use_global = False, num_candidates = 50000, device='cpu'):
#     import torch
#     import numpy as np
...
#     # Baseline setup to isolate local network tracking footprint
#     src_true_init = np.array([np.mean(domain_lat_range), np.mean(domain_lon_range), np.mean(domain_depth_range)]).reshape(1, -1)
#     src_true_init_xyz = torch.tensor(ftrns1(src_true_init), device=device, dtype=torch.float32)
#     all_dists_init = torch.cdist(src_true_init_xyz, station_xyz).squeeze(0)
#     _, sta_idx_init = torch.topk(all_dists_init, k=k_stations, largest=False)
...
#     # --- FIXED: Vectorized WGS84 Ellipsoidal Travel Time Math ---
#     a, b = 6378137.0, 6356752.3142
#     sta_norm = active_stas_xyz / torch.norm(active_stas_xyz, dim=1, keepdim=True)
#     src_true_norm = src_true_xyz / torch.norm(src_true_xyz, dim=1, keepdim=True)
...
#     # --- KEEPING YOUR PREFERRED BEHAVIOR: Box-Intersection Suppression Gate ---
#     d_space_true = torch.norm(cand_xyz - src_true_xyz, dim=1)
#     d_time_true = torch.abs(cand_dt - 0.0)
...
# def probe_network_sidelobes_geodetic4(station_latlonz, domain_lat_range, domain_lon_range, domain_depth_range, ftrns1, ftrns2,
...
#     # # --- ADAPTIVE VELOCITY SCALING ENGINE ---
#     # if local_aperture < 300000.0:  # Regional/Local Crustal (< 300 km)
#     #     vel_avg = 3500.0
#     #     vel_min = 2500.0
#     # elif local_aperture < 1500000.0: # Large Continental/Upper Mantle (< 1500 km)
#     #     vel_avg = 6000.0
#     #     vel_min = 4500.0
#     # else:                             # Global/Deep Mantle & Core Tele-seismic
#     #     vel_avg = 10000.0
#     #     vel_min = 8000.0
...
#     # Vectorized WGS84 Ellipsoidal Travel Time Math
#     a, b = 6378137.0, 6356752.3142
#     sta_norm = active_stas_xyz / torch.norm(active_stas_xyz, dim=1, keepdim=True)
#     src_true_norm = src_true_xyz / torch.norm(src_true_xyz, dim=1, keepdim=True)
...
#     # --- FIXED: Scale-Agnostic Travel Time Engine ---
#     if local_aperture < 300000.0:
#         # Local/Regional Mode: Use true 3D Euclidean distances to preserve depth (Z)
#         t_obs = torch.norm(active_stas_xyz - src_true_xyz, dim=1) / vel_avg
#         t_calc = torch.cdist(cand_xyz, active_stas_xyz) / vel_avg
#     else:
#         # Global Mode: Use WGS84 Ellipsoidal Surface Arc Distances
#         a, b = 6378137.0, 6356752.3142
#         sta_norm = active_stas_xyz / torch.norm(active_stas_xyz, dim=1, keepdim=True)
#         src_true_norm = src_true_xyz / torch.norm(src_true_xyz, dim=1, keepdim=True)
#         cand_norm = cand_xyz / torch.norm(cand_xyz, dim=1, keepdim=True)
