...
def build_graphs_domain(m_domain, locs_use, stas_use, scale_domain, deg_padding, number_of_spatial_nodes, k_spc_edges, k_sta_edges, depth_range, ftrns1, ftrns2, use_global = False, assign_based_on_grid = False, max_nodes = 3000, n_trgt_nodes = 200e3, Vc = 6500.0, max_time_shift_range = None, file_index = 0, date = [2000, 1, 1], n_grids = 1, initialize = None, fixed_domain = None, use_paths = False, rbest = None, mn = None, optimize_station_graphs = False, optimize_source_graphs = False, use_domain_approximate = False, use_tuner = True, n_tuner_steps = 50, name_of_project = '', verbose = True, save_file = True, device = 'cpu'):
	...
	if (use_domain_approximate == True) and (m_domain is not None):
		...
		if initialize is not None:
			...
			domain_params = np.load(file_load)
		...
		scale_time = domain_params['scale_time']
		depth_boost = domain_params['depth_boost']
		locs_use = domain_params['locs_use']
		stas_use = domain_params['stas_use']
		x_grid = domain_params['x_grid']
		lat_range = domain_params['lat_range']
		lon_range = domain_params['lon_range']
		lat_range_extend = domain_params['lat_range_extend']
		lon_range_extend = domain_params['lon_range_extend']
		depth_range = domain_params['depth_range']
		...
		source_label_width = domain_params['source_label_width']
		source_label_width_t = domain_params['source_label_width_t']
		association_label_width = domain_params['association_label_width']
		association_label_width_t = domain_params['association_label_width_t']
		sigma_input = domain_params['sigma_input']
		if initialize is not None: z.close()
...
# def fit_spatial_domain(locs_use, stas_use, scale_domain, deg_padding, number_of_spatial_nodes, k_spc_edges, k_sta_edges, depth_range, ftrns1, ftrns2, use_global = False, max_nodes = 3000, n_trgt_nodes = 200e3, Vc = 6500.0, file_index = 0, date = [2000, 1, 1], rbest = None, mn = None, domain = None, initialize = None, fixed_domain = None, max_time_shift_range = None, min_time_range = 3.0, n_rand_srcs = 250, quantile_times = 0.6, quantile_times_srcs = 0.6, extend_ratio = 2.0, use_tuner = True, n_grids = 1, n_tuner_steps = 50, verbose = True, device = 'cpu'):
...
def fit_spatial_domain(locs_use, stas_use, scale_domain, deg_padding, number_of_spatial_nodes, k_spc_edges, k_sta_edges, depth_range, ftrns1, ftrns2, use_global = False, max_nodes = 3000, n_trgt_nodes = 200e3, Vc = 6500.0, file_index = 0, date = [2000, 1, 1], rbest = None, mn = None, domain = None, initialize = None, fixed_domain = None, max_time_shift_range = None, min_time_range = 3.0, n_rand_srcs = 250, quantile_times = 0.25, quantile_times_srcs = 0.25, extend_ratio = 2.0, use_tuner = True, n_grids = 1, n_tuner_steps = 50, verbose = True, device = 'cpu'):
	...
	if use_previous_approach == True:
		...
		for i in range(n_rand_srcs):
			...
			k_stations = int(np.random.randint(min_k, max_k + 1))
...
# 			probs = exp_logits / sum_exp if sum_exp > 0 else np.ones(n_stas_total) / n_stas_total
...
# 		cluster_stas = st_ecef[chosen_idx]
# 		aperture_s = float(np.percentile(cdist(cluster_stas, cluster_stas), 85)) if len(cluster_stas) > 1 else 1000.0
# 		cluster_apertures.append(aperture_s)
...
# 				V_adaptive = 13000.0
# 		else:
# 			V_adaptive = Vs if Vs is not None else vel_phase
# 		source_velocities[s] = V_adaptive
...
# 	# 3. Generate Geodetically Stable Local Perturbations (Scale-Invariant Log-Uniform Sampling)
# 	lat_m_per_deg = 111000.0
	
# 	log_fracs = np.random.uniform(-3.0, 0.0, (n_srcs, n_test_per_src))
# 	scale_factors = 10 ** log_fracs

# 	angles = np.random.normal(size=(n_srcs, n_test_per_src, 3))
# 	angles /= np.maximum(np.linalg.norm(angles, axis=-1, keepdims=True), 1e-12)

# 	dx_m = angles[:, :, 0] * local_scales[:, None] * scale_factors
# 	dy_m = angles[:, :, 1] * local_scales[:, None] * scale_factors
# 	dz_m = angles[:, :, 2] * (local_scales[:, None] * 0.2) * scale_factors
	
# 	time_offs = np.random.uniform(-1, 1, (n_srcs, n_test_per_src)) * (time_limits[:, None] * scale_factors)
...
# 		# Reference travel time for source s -> explicitly force shape (k_s,)
# 		t_r_tensor = trv(s_stas, src_refs_lla[s:s+1], V_wave=V_s)
# 		t_r = t_r_tensor.cpu().detach().numpy().reshape(-1)  # Always 1D array of length k_s
			
# 		noise_values = generate_travel_time_noise(t_r, phase_input="P")[0]
# 		t_obs_s = t_r + noise_values.reshape(t_r.shape)  # Shape: (k_s,)
		
# 		# Test travel time for perturbed points -> explicitly force shape (n_test_per_src, k_s)
# 		test_pts_s = test_lla_flat[s * n_test_per_src : (s + 1) * n_test_per_src]
# 		t_t_tensor = trv(s_stas, test_pts_s, V_wave=V_s)
# 		t_test_s = t_t_tensor.cpu().detach().numpy().reshape(n_test_per_src, -1)
		
# 		# Residuals across active stations
# 		residuals_s = t_obs_s[None, :] - t_test_s - time_offs[s, :, None]
# 		sigma_d_s = np.maximum(t_test_s * noise_level, 1e-6)
		
# 		# Reduced Chi-Square misfit across stations
# 		chi_s = np.sqrt(np.mean((residuals_s / sigma_d_s)**2, axis=1))
# 		chi_error_list.append(chi_s)

# 	chi_error = np.stack(chi_error_list, axis=0)

# 	# 6. Extract Widths with Robust Percentile Slope Selection
# 	dist_s = np.sqrt(dx_m**2 + dy_m**2 + dz_m**2) 
# 	mask = (chi_error > 0.05) & (chi_error < 2.0)

# 	if np.any(mask):
# 		w_phys = float(np.percentile(dist_s[mask] / chi_error[mask], 25))
# 		w_t = float(np.percentile(np.abs(time_offs)[mask] / chi_error[mask], 25))
# 	else:
# 		w_phys = float(np.mean(local_scales) * 0.05)
# 		w_t = float(np.mean(time_limits) * 0.05)
...
#							noise_level=0.02, n_srcs=250, n_test_per_src=10000, 
#							k_range=None, use_global=False, Vs=None, 
#							overwrite_vs=True, dominant_freq_hz=2.0, device='cpu'):
#	 import torch
#	 import numpy as np
#	 from scipy.spatial.distance import cdist
...
#	 # 2. Calculate Search Limits & Adaptive Parameters
#	 all_dists = cdist(ref_ecef, st_ecef)
#	 nearest_idx = []
#	 local_scales = np.zeros(n_srcs)
#	 time_limits = np.zeros(n_srcs)
...
#	 # 3. Local Perturbations (Scale-Invariant Log-Uniform Sampling)
#	 lat_m_per_deg = 111000.0
#	 log_fracs = np.random.uniform(-3.0, 0.0, (n_srcs, n_test_per_src))
#	 scale_factors = 10 ** log_fracs
...
#	 # 4 & 5. Backprojection Stacking Coherence (PSF Beam Response)
#	 stack_coherence_list = []
#	 omega_dom = 2.0 * np.pi * dominant_freq_hz
...
#		 t_r_tensor = trv(s_stas, src_refs_lla[s:s+1], V_wave=V_s)
#		 t_r = t_r_tensor.cpu().detach().numpy().reshape(-1)  # (k_s,)
			
#		 noise_values = generate_travel_time_noise(t_r, phase_input="P")[0]
#		 t_obs_s = t_r + noise_values.reshape(t_r.shape)  # Observed travel times
		
#		 test_pts_s = test_lla_flat[s * n_test_per_src : (s + 1) * n_test_per_src]
#		 t_t_tensor = trv(s_stas, test_pts_s, V_wave=V_s)
#		 t_test_s = t_t_tensor.cpu().detach().numpy().reshape(n_test_per_src, -1)  # (10000, k_s)
		
#		 # Differential travel times including trial origin time shift
#		 dt_s = (t_obs_s[None, :] - t_test_s) - time_offs[s, :, None]
		
#		 # Harmonic phase stack over active stations (Backprojection PSF)
#		 # B(x, t) = | 1/K * sum_k exp(i * omega * dt_k) |
#		 phase_stack = np.mean(np.exp(1j * omega_dom * dt_s), axis=1)
#		 coherence_s = np.abs(phase_stack)
#		 stack_coherence_list.append(coherence_s)

#	 coherence_map = np.stack(stack_coherence_list, axis=0)  # Shape: (n_srcs, n_test_per_src)

#	 # 6. Extract Widths at the 1/e (-3.7 dB) PSF Beam Envelope Contour
#	 dist_s = np.sqrt(dx_m**2 + dy_m**2 + dz_m**2)
#	 abs_dt = np.abs(time_offs)

#	 # Isolate trial grid locations near the 1/e beam contour drop-off
#	 psf_target = 1.0 / np.e  # ~0.3679 beam amplitude threshold
#	 contour_mask = (coherence_map >= psf_target * 0.85) & (coherence_map <= psf_target * 1.15)

#	 if np.any(contour_mask):
#		 w_phys = float(np.median(dist_s[contour_mask]))
#		 w_t = float(np.median(abs_dt[contour_mask]))
#	 else:
#		 # Physical lower bound fallback based on Rayleigh-Fresnel resolution limit
#		 wavelength_m = vel_phase / dominant_freq_hz
#		 w_phys = float(0.5 * np.sqrt(wavelength_m * aperture_m))
#		 w_t = float(0.5 / dominant_freq_hz)

#	 rel_scale = w_phys / aperture_m

#	 print(f"\n{'='*55}")
#	 print(f"BACKPROJECTION PSF KERNEL ESTIMATION")
#	 print(f"{'='*55}")
#	 print(f"Total Points Sampled:   {n_srcs * n_test_per_src:,}")
#	 print(f"Dominant Frequency:	 {dominant_freq_hz:.2f} Hz")
#	 print(f"Spatial Width (W_phys): {w_phys/1000:.4f} km")
#	 print(f"Temporal Width (W_t):   {w_t:.4f} s")
#	 print(f"Computed Aperture:	  {aperture_m/1000:.2f} km")
#	 print(f"Relative Resolution:	{rel_scale:.4%}\n")
...
def estimate_kernel_widths(domain, station_locs, z_range=(-40000, 2000), vel_phase=6000.0, 
						   noise_level=0.02, n_srcs=250, n_test_per_src=10000, 
						   k_range=None, use_global=False, Vs=None, 
						   overwrite_vs=True, dominant_freq_hz=None, device='cpu'):
	import torch
	import numpy as np
	from scipy.spatial.distance import cdist
	...
	# --- Continuous Sigmoid Ramp for Dominant Frequency ---
	if dominant_freq_hz is None:
		# Anchor points (Aperture in meters)
		a_min = 10_000.0	 # 10 km (Local)
		a_max = 2_000_000.0  # 2,000 km (Global)

		# Frequency bounds (Hz)
		f_max = 8.0   # Local micro-seismic cap
		f_min = 0.20  # Global teleseismic floor

		# Log-center and transition steepness
		log_a = np.log10(np.clip(aperture_m, a_min, a_max))
		log_a_min = np.log10(a_min)
		log_a_max = np.log10(a_max)

		# Normalized scale [0, 1] across log-aperture
		norm_val = (log_a - log_a_min) / (log_a_max - log_a_min)

		# Smooth Cosine/Sigmoid decay from f_max to f_min
		weight = 0.5 * (1.0 + np.cos(np.pi * norm_val))
		dominant_freq_hz = f_min + (f_max - f_min) * weight
...
def probe_network_sidelobes_geodetic_backup(
	...
	if local_aperture < 300000.0:
		...
		if add_travel_times_noise == True:
