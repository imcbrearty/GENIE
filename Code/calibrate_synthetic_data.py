...
def radial_function(points, center, inv_cov, sigma_radial, p_exp, scale_factor):
	...
	# assert(np.abs(radial_pdfdf - radial_pdf1).max() < 1e-2)
...
def invert_probabilities(radial_pdf, noise, lambda_corr=0.5, sigma_logistic=1.0, decaying_factor=300):
    ...
    def g_step(radial_pdf, decaying_factor=300):  # decaying_factor = 25 => decays from 0.25
...
def sample_synthetic_moveout_pattern_generator(locs, Srcs, Mags, Inds, prob_vec, chol_params, ftrns1, pdist_p, pdist_s, n_samples = 100, use_l1 = False, mask_noise = False, return_features = True, use_ichoose=True): # n_repeat : can repeatedly draw either from the covariance matrices, or the binomial distribution

	## Sample sources
	if use_ichoose == True:
		ichoose = np.random.choice(len(Srcs), p = prob_vec, size = n_samples)
	else:
		ichoose = np.arange(len(Srcs))[:n_samples]
...
def evaluate_bayesian_objective_evaluate(x, n_batch = 100, n_mag_bins = 5, return_config = False):
	...
	## Sample a generation
	# st_time = time.time()
	srcs_sample, mags_sample, features, ind_sample, [ikeep_p1, ikeep_p2, ikeep_s1, ikeep_s2] = sample_synthetic_moveout_pattern_generator(locs, Srcs, Mags, Inds, prob_vec, chol_params, ftrns1, pdist_p, pdist_s, n_samples = n_batch)
...
if __name__ == "__main__":
	...
	for i in range(len(st_list)):
		...
		if check_for_glitches == True:
			...
			for j in range(spatial_clusters):
				...
				#ikeep thresh = np.quantile(counts, np.arange(0, 1.0001, 0.0001))[-8] # 2.0
	...
	## Sample a generation
	st_time = time.time()
	srcs_sample, mags_sample, features, ind_sample, [ikeep_p1, ikeep_p2, ikeep_s1, ikeep_s2] = sample_synthetic_moveout_pattern_generator(locs,Srcs, Mags, Inds, prob_vec, chol_params, ftrns1, pdist_p, pdist_s, n_samples = n_batch, return_features = False)
	...
	bounds = [(1.5, 5.0), # p_exponent
			(0.5, 1.0), # scale_factor
			(1e3, 100e3), # sigma_noise
			(0.1, 15.0), # sigma_radial_divider
			(0.1, 10.0), # threshold_logistic
			(0.01, 0.5), # lambda_corr
			(0.1, 5.0), # sigma_radial_p_factor
			(0.1, 5.0), # sigma_radial_s_factor
			(0.0, np.pi/2.0), # angle_perturbation
			(0.0, 0.4), # length_perturbation
			(0.0, 0.40), # miss_pick_rate
