...
for cnt, strs in enumerate([0]):
	...
	scale_time_ref = 3500.0
	tree_refined = cKDTree(np.concatenate((ftrns1(srcs_refined), scale_time_ref*srcs_refined[:,[3]]), axis = 1))
	ip_retained = tree_refined.query(np.concatenate((ftrns1(srcs_refined_1), scale_time_ref*srcs_refined_1[:,[3]]), axis = 1))[1]
