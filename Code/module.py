...
# 		misfit_rel_time = torch.cat((torch.exp(-0.5*(misfit_rel_time**2)/(((2.0*self.kernel_sig_t)**2))), torch.sign(misfit_rel_time)), dim = 1)
# 		misfit_query_time = torch.cat((torch.exp(-0.5*(misfit_query_time**2)/(((2.0*self.kernel_sig_t)**2))), torch.sign(misfit_query_time)), dim = 1)
...
# 		## degree_srcs, cum_degree_srcs
# 		arv_embed = self.SourceArrivalEmbedding(s, x_temp_cuda_cart, x_temp_cuda_t, x_query_src_cart, tq_sample, A_src_in_sta, tpick, ipick, phase_label, locs_use_cart, tlatent, trv_out = trv_out_q)
