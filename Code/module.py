...
class LocalSliceLgCollapse(MessagePassing):
    ...
    def forward(self, A_edges, dt_partition, tpick, ipick, phase_label, inpt, tlatent, n_temp, n_sta, k_infer = 10): # reference k nearest spatial points
        ...
        # dt_partition is now a numpy array - extract dt directly to avoid GPU precision issues
        dt = float(dt_partition[1] - dt_partition[0])

        if self.use_phase_types == False:
            phase_label = phase_label*0.0

    
        t_index = torch.floor((tpick - dt_partition[0]) / dt).long() # index into A_edges, which is each station, each dt_point, each k.
        t_index = ((ipick*l_dt*k_infer + t_index*k_infer).view(-1,1) + torch.arange(k_infer).view(1,-1).to(self.device)).reshape(-1).long() # check this
...
class StationSourceAttentionMergedPhases(MessagePassing):
    ...
    def forward(self, src, stime, src_embed, trv_src, locs_cart, arrival_p, arrival_s, tpick, ipick, phase_label): # reference k nearest spatial points
        ...
        if (self.use_neighbor_assoc_edges == True)*(len(ipick) > (self.k_sta_edges + 1)):
            ...
            if dense_array == True:
                locs_cart_np = locs_cart.detach().cpu().numpy()
                densities = compute_local_density(locs_cart_np, density_radius)
                dense_edges = np.array(create_edges_for_dense_nodes(locs_cart_np, densities, base_K=base_K, C=C, base_p=base_p, R_far=R_far, low_density_thresh=low_density_thresh), dtype = np.int64).reshape(-1, 2)
                k_edges = remove_self_loops(torch.from_numpy(dense_edges.T).to(self.device))[0].flip(0).contiguous()
            else:
                k_edges = remove_self_loops(knn(locs_cart/1000.0, locs_cart/1000.0, k = self.k_sta_edges + 1))[0].flip(0).contiguous()
