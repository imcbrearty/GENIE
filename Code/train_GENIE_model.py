...
def pick_labels_extract_interior_region(xq_src_cart, xq_src_t, source_pick, src_slice, lat_range_interior, lon_range_interior, ftrns1, sig_x = 12.5e3, sig_t = 6.5): # can expand kernel widths to other size if prefered
...
for i in range(n_restart_step, n_epochs):
	...
	if (i == n_restart_step)*(n_restart == True):
		...
		mz.load_state_dict(torch.load(write_training_file + 'trained_gnn_model_step_%d_ver_%d.h5'%(n_ver, n_restart_step)))
		optimizer.load_state_dict(torch.load(write_training_file + 'trained_gnn_model_step_%d_ver_%d_optimizer.h5'%(n_ver, n_restart_step)))
		zlosses = np.load(write_training_file + 'trained_gnn_model_step_%d_ver_%d_losses.h5'%(n_ver, n_restart_step))
	...
	if np.mod(i, 100) == 0:
		torch.save(mz.state_dict(), write_training_file + 'trained_gnn_model_step_%d_ver_%d.h5'%(n_ver, n_restart_step))
		torch.save(optimizer.state_dict(), write_training_file + 'trained_gnn_model_step_%d_ver_%d_optimizer.h5'%(n_ver, n_restart_step))
		np.savez_compressed(write_training_file + 'trained_gnn_model_step_%d_ver_%d_losses.h5'%(n_ver, n_restart_step), losses = losses, mx_trgt_1 = mx_trgt_1, mx_trgt_2 = mx_trgt_2, mx_trgt_3 = mx_trgt_3, mx_trgt_4 = mx_trgt_4, mx_pred_1 = mx_pred_1, mx_pred_2 = mx_pred_2, mx_pred_3 = mx_pred_3, mx_pred_4 = mx_pred_4, scale_x = scale_x, offset_x = offset_x, scale_x_extend = scale_x_extend, offset_x_extend = offset_x_extend, training_params = training_params, graph_params = graph_params, pred_params = pred_params)
