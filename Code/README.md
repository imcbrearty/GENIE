...
The training speed can be improved by first building the training data using "generate_synthetic_data", saving the data to distinct files, and loading during training (the memory cost of the files can be ~Tb in size). Currently it builds a new batch of training data between each update step.
