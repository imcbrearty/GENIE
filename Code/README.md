...
The training speed can be improved by first building the training data using generate_synthetic_data, saving the data to distinct files, and loading during training (the memory cost of the files can be ~Tb in size). Use the parameter "fixed_subnetworks" in train_config.yaml to train the GNN on specific instances of subnetworks available per day, as recorded in the pick data (else, only random subsets of stations are chosen).
