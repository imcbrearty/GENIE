...
if compute_reference_times == True:
	...
	if num_cores == 1:
		...
		for j in ind_use:
			...
			output_data = dict(loc = locs_ref[j], locs_ref = locs_ref, Tp_interp = tp, Ts_interp = ts)
			if save_dense_travel_time_data or not train_travel_time_neural_network:
				output_data.update(X = X, xx = xx, Tp = results[0], Ts = results[1], Vp = Vp, Vs = Vs, Vp_interp = Vp_interp, Vs_interp = Vs_interp, depth_grid = depth_grid)

			np.savez_compressed(path_to_file + '1D_Velocity_Models_Regional' + seperator + 'TravelTimeData' + seperator + '%s_1d_velocity_model_station_%d_ver_%d.npz'%(name_of_project, j, vel_model_ver), **output_data)
