...
def pairwise_geodesic_distance_3d1(sources, stations, max_iter=200, tol=1e-12):
    """
    ...
    elev2 = stations[:, 2][np.newaxis, :] # Elevation in meters
    ...
    sin_sigma = np.zeros_like(L)
    cos_sigma = np.zeros_like(L)
    sigma = np.zeros_like(L)
    sin_alpha = np.zeros_like(L)
    cos2_alpha = np.zeros_like(L)
    cos2_sigma_m = np.zeros_like(L)
    ...
    for _ in range(max_iter):
        ...
        lambda_lon = L + (1.0 - C) * f * sin_alpha * (
            sigma + C * sin_sigma * (
                cos2_sigma_m + C * cos_sigma * (-1.0 + 2.0 * cos2_sigma_m ** 2)
            )
        )

        diff = np.abs(lambda_lon - lambda_prev)
        converged |= (diff < tol)
...
def generate_travel_time_noise(
    ...
    gamma_path=0.165, # 0.12  # Path noise growth coefficient (sec / sqrt(sec))
    scale_extra=1.0,
...
def perturb_wgs84(
    ...
    time_shift_range,
    is_global_lon=True,
    sampling_mode="normal",  # Options: "normal" (default, backward-compatible), "clipped_normal", "uniform"
    clip_sigma=2.0,          # Truncation threshold for "clipped_normal"
    a=6378137.0,
    e=8.18191908426215e-2,
):
    """Refactored WGS84 perturbation using exact 3D ECEF tangent rotation.

    Supports three perturbation sampling modes:
    - 'normal': Unclipped Gaussian draws (original behavior, best for training target generation).
    - 'clipped_normal': Gaussian draws truncated at clip_sigma * kernel_size (prevents far-tail queries).
    - 'uniform': Uniform random sampling within exact bounding box (recommended for post-processing refinement).
    """
    n_pts = len(base_lat_deg)
    if n_pts == 0:
        return np.empty(0), np.empty(0), np.empty(0), np.empty(0)
    ...
    # 1. Generate local perturbations based on chosen strategy
    if sampling_mode == "normal":
        # Original behavior: Unclipped Gaussian sampling
        dE = np.random.normal(0, src_x_kernel_m, size=n_pts)
        dN = np.random.normal(0, src_x_kernel_m, size=n_pts)
        dz = np.random.normal(0, src_depth_kernel_m, size=n_pts)
        dt = np.random.normal(0, src_t_kernel, size=n_pts)
        ...
        # Uniform sampling within search box
        ...
        raise ValueError(f"Unknown sampling_mode: '{sampling_mode}'. Options are 'normal', 'clipped_normal', or 'uniform'.")

    # 2. Convert Base points to ECEF
    lla_base = np.column_stack((base_lat_deg, base_lon_deg, base_depth_m))
    ecef_base = lla2ecef(lla_base, a=a, e=e)
    ...
    # 7. Apply Depth & Time Perturbations
    new_depth = reflect_bounds(base_depth_m + dz, depth_range[0], depth_range[1])
    new_t = reflect_bounds(base_t_s + dt, -half_t_window, half_t_window)
