...
def evaluate_bayesian_objective(x, n_random = 30, t_sample_win = 120.0, windows = [40e3, 150e3, 300e3], t_win_ball = [10.0, 15.0, 25.0], return_vals = False): # 	windows = [40e3, 150e3, 300e3], 	
	...
	training_params_2[5][1] = x[1] # coda_win

	training_params_3[1][0] = x[2] # dist_range[0]
	training_params_3[1][1] = x[3] # dist_range[1]
	training_params_3[2] = x[4] # max_rate_events
	training_params_3[4] = x[4]*x[5] # max_false_events (input of false rate to generate is an absolute number, not the ratio, which is x[5])
...
print(f"parameters: {x_optimal}")
np.savez_compressed(path_to_file + 'Grids/%s_optimized_stats_parameters_ver_%d.npz'%(name_of_project, n_ver_optimize), res = res, x = np.array(optimize.x), arrivals = arrivals)
print('Optimal training data parameters loss: %0.4f'%(res))
print("Parameters")
print(x_optimal)

strings = ['coda_rate', 'coda_win', 'dist_range[0]', 'dist_range[1]', 'max_rate_events', 'max_false_events']

## Print parameter values
print('\nOptimized Parameters:')
for i, (param_name, param_value) in enumerate(zip(strings, x_optimal)):
	print(f'{param_name}: {param_value:.6f}')

## Save with timestamp
from datetime import datetime
date = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
np.savez_compressed(path_to_file + 'Grids/%s_optimized_stats_parameters_%s_loss_%0.4f.npz' % (name_of_project, date, res), x = np.array(optimize.x), res=res, arrivals = arrivals, strings = strings)

print('\nFinished optimized training data')
